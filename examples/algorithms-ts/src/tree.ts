...
export class LCA {
  ...
  constructor(adj: TreeAdj, root = 0) {
    this.adj = adj;
    this.n = adj.length;
  ...
  private dfs(u: number, p: number): void {
    this.up[u][0] = p;
...
export class HeavyLightDecomposition {
  ...
  getPosition(u: number): number { return this.pos[u]; }
  getHead(u: number): number { return this.head[u]; }
  getDepth(u: number): number { return this.depth[u]; }
}

// ==================== 重心分解 ====================

export function centroidDecomposition(adj: TreeAdj): number[] {
  const n = adj.length;
  const removed = new Array(n).fill(false);
  const subtrees: number[] = [];

  function calcSizes(u: number, p: number, sizes: number[]): void {
    sizes[u] = 1;
  ...
  function findCentroid(u: number, p: number, total: number, sizes: number[]): number {
    for (const v of adj[u]) {
 ...
 * 时间复杂度: O(n)
 */
export function cartesianTree(arr: number[]): { parent: number[]; root: number } {
  const n = arr.length;
...
export function runTreeTests(): void {
  runTests("Tree", {
    "LCA": () => {
      const adj: TreeAdj = [[1, 2], [3, 4], [5], [], [], []];
      const lca = new LCA(adj, 0);
      assertEq(lca.query(3, 4), 1);
      assertEq(lca.query(3, 5), 0);
      assertEq(lca.query(1, 2), 0);
    },
