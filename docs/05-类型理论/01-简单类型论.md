...
- [4. 类型推导理论](#4-类型推导理论)
  - [4.1 主类型理论](#41-主类型理论)
  - [4.2 统一算法](#42-统一算法)
  - [4.3 Hindley-Milner类型系统 \[Milner1978\]](#43-hindley-milner类型系统-milner1978)
- [5. 语义理论](#5-语义理论)
  - [5.1 指称语义](#51-指称语义)
  - [5.2 环境语义](#52-环境语义)
  - [5.3 语义函数](#53-语义函数)
...
简单类型论的公理化表述可追溯至 Church 的简单类型λ演算 [Church1940]。**公理 1.1.1** (类型系统公理) 类型系统是一个五元组：
**Axiom 1.1.1** (Type System Axiom) A type system is a 5-tuple [Church1940]:
...
**解释与直观**：简单类型论将项与类型用 $\lambda$ 演算与类型规则形式化；$\Gamma \vdash e : \tau$ 表示在上下文 $\Gamma$ 下项 $e$ 具有类型 $\tau$，为依赖类型与类型系统（见 02、04）提供基础。

**概念属性表**：类型 $\tau$ — 基类型与函数类型 $\tau_1 \to \tau_2$；推导 $\Gamma \vdash e : \tau$ — 可弱化、可替换；Curry-Howard — 命题即类型、证明即项。

**概念关系**：简单类型论 —depends_on— λ演算（见 §2）；与依赖类型论 — 后者类型可依赖值；与 04-类型系统 — 类型检查与推导（见 04）。

**概念依赖图**：λ演算 → 项与规约；类型规则 → $\Gamma \vdash e : \tau$；Curry-Howard → 03-形式化证明、06-逻辑系统。

**思维导图**：简单类型论 → 类型、项、推导、Curry-Howard → 依赖类型、类型系统、证明助手。

**多维矩阵**：简单/多态/依赖 — 类型论层次；Hindley-Milner — 类型推导；进展与保持 — 类型安全（见 04）。

**公理定理推理证明决策树**：类型规则定义 → 可弱化定理；进展与保持 → 类型安全（见 04）；Curry-Howard → 命题逻辑对应。

**应用决策建模树**：类型检查/推导 → 见 04-类型系统；形式化证明 → 见 02-依赖类型、08-Lean/Coq；程序设计 → 见 08-实现示例。
...
### 4.3 Hindley-Milner类型系统 [Milner1978]

**定义 4.3.1** (多态类型) Hindley-Milner类型系统扩展了简单类型 [Milner1978]：
...
> **2024-2025 研究进展**：语义类型可靠性方法在 Iris 框架下的进一步发展 [LogicalTypeSoundness2024] 为高阶语言的类型安全证明提供了更强大的逻辑工具；Lean 4.8.0 [Lean4802024] 在 defeq 与 WHNF 算法上的改进也直接影响了简单类型系统的实现效率；Martin-Löf 类型论的 Coq 机械化 [MartinLof2024] 则为从简单类型到依赖类型的元理论迁移奠定了形式化基础。
...
5. [DamasMilner1982] Damas, L., & Milner, R. (1982). "Principal Type-Schemes for Functional Programs". *POPL '82*, 207-212.
...
1. [LogicalTypeSoundness2024] Jung, R., et al. (2024). "A Logical Approach to Type Soundness". *Journal of the ACM*. DOI: 10.1145/3676954.
   - 使用 Iris 高阶分离逻辑框架进行语义类型可靠性证明，为类型系统元理论提供新工具。

2. [Lean4802024] The Lean Development Team. (2024). "Lean 4.8.0 Release Notes". GitHub Release.
   - 包含 370+ 项改进，涉及 defeq/WHNF 算法、终止性检查与类型系统优化。

3. [MartinLof2024] Adjedj, A., et al. (2024). "Martin-Löf à la Coq". *CPP 2024*.
   - 在 Coq 中完整机械化 Martin-Löf 类型论元理论，证明类型检查与转换的可判定性。
...
- `view/算法全景梳理-2025-01-11.md` - 算法全景梳理（包含类型理论概述）
...
- `view/算法全景梳理-2025-01-11.md` §2.1 - 计算模型维度（λ演算、类型系统）
- `view/VIEW内容总索引-2025-01-11.md` §2.1 - 内容主题索引（类型理论）
- `view/VIEW内容总索引-2025-01-11.md` - VIEW文件夹完整索引
