...
- **依赖类型系统与数理逻辑**: `05-类型理论/05-依赖类型系统与数理逻辑.md` §5.2-§5.4 - 强依赖
...
- **直观**：先理解「类型即空间、证明即路径」的直观（§3.1–§3.2）；类型等价与路径类型的几何直觉（§3.3）；圆、球面等高阶归纳类型的直观构造（§3.4）。
- **形式**：再掌握路径类型 `a =_A b`、统一公理 (A ≃ B) → (A = B)、高阶归纳类型的形式规则（§3.2–§3.4）；形式化证明系统（§3.5）。
- **应用**：结合 [10-高级主题/01-范畴论在计算中的应用](../10-高级主题/01-范畴论在计算中的应用.md)、[08-实现示例/03-Lean实现](../08-实现示例/03-Lean实现.md) 理解 HoTT 在证明助手与统一基础中的应用。

### 复习要点 / Review Points

- **关键概念**：同伦类型论、类型即空间、路径类型、类型等价、统一公理、高阶归纳类型（§3.1–§3.4）。
- **关键定理/性质及位置**：路径操作与同伦群 §3.2；等价性质 §3.3；高阶归纳类型（圆、球面、环面）§3.4。
- **与它模块衔接**：依赖类型论见 05-02；范畴视角见 10-01-范畴论在计算中的应用。
...
- [3.3 类型等价 (Type Equivalence)](#33-类型等价-type-equivalence)
  ...
  - [3.3.4 路径类型的高级应用 (Advanced Applications of Path Types)](#334-路径类型的高级应用-advanced-applications-of-path-types)
...
- [3.5 形式化证明系统 (Formal Proof Systems)](#35-形式化证明系统-formal-proof-systems)
  ...
  - [3.5.4 Agda 中的 HoTT/UF 实践 (HoTT/UF Practice in Agda)](#354-agda-中的-hottuf-实践-hottuf-practice-in-agda)
...
同伦类型论是类型论与同伦论的结合，它将类型视为空间，将类型等价视为同伦等价 [HoTTBook2013]；Voevodsky 提出的统一基础（Univalent Foundations）为其提供了公理化框架 [Voevodsky2014]。

Homotopy Type Theory is the combination of type theory and homotopy theory, where types are viewed as spaces and type equivalences as homotopy equivalences [HoTTBook2013]; the Univalent Foundations proposed by Voevodsky provide its axiomatic framework [Voevodsky2014].
...
### 3.3.4 路径类型的高级应用 (Advanced Applications of Path Types)

**路径类型在证明中的应用 / Path Types in Proofs:**

路径类型 $a =_A b$ 不仅表示相等性，在证明中可用于：

- **传递性**: 若 $p : a = b$ 且 $q : b = c$，则 $p \cdot q : a = c$
- **函数外延性**: 证明两个函数在逐点相等时相等
- **单值公理**: 等价类型相等 $(A \simeq B) \rightarrow (A = B)$，将等价提升为恒等类型

**同伦等价的实际应用 / Practical Applications of Homotopy Equivalence:**

- **类型等价证明**: 通过构造双向映射与同伦证明 $A \simeq B$
- **高阶归纳类型的消去**: 沿路径计算，得到依赖类型的传输
- **同伦群计算**: 利用 $\pi_n(S^n) \cong \mathbb{Z}$ 等结论进行形式化

**交叉引用**: 高阶归纳类型与路径构造参见 `05-类型理论/05-依赖类型系统与数理逻辑.md` §5.4.2（高阶归纳类型）。
...
### 3.5.4 Agda 中的 HoTT/UF 实践 (HoTT/UF Practice in Agda)

**教程与资源 / Tutorial and Resources:**

Martín Escardó 的 **Introduction to Homotopy Type Theory and Univalent Foundations (HoTT/UF) with Agda**（2025 年 11 月更新）提供：

- 使用 Agda 开发单值数学的字面编程教程
- 形式化、机器验证的数学发展
- HTML、PDF 及 arXiv 版本

**单值基础的实际应用 / Practical Univalent Foundations:**

- **类型等价与恒等**: 在 Agda 中利用单值公理将等价类型视为相等，简化类型层面的推理
- **高阶归纳类型**: 圆、球面、环面等在 Agda 中的定义与消去规则
- **与证明助手的衔接**: Agda 的 HoTT 模式与 Cubical Agda 支持路径类型与单值公理

**交叉引用**: 课程与教程引用参见 `docs/项目权威资源对齐分析报告-2025.md`；依赖类型与证明助手实践参见 `05-类型理论/05-依赖类型系统与数理逻辑.md` §5.6、§5.8。
