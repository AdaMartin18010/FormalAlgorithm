...
1. [Robinson1965] Robinson, J. A. (1965). "A Machine-Oriented Logic Based on the Resolution Principle". *Journal of the ACM*, 12(1): 23-41. DOI: 10.1145/321250.321253.
   - 统一算法的经典论文，类型推导的核心组件。本文档§3.1的统一算法基于此论文。

2. [DamasMilner1982] Damas, L., & Milner, R. (1982). "Principal Type-Schemes for Functional Programs". *POPL '82*, 207-212. DOI: 10.1145/582153.582176.
   - Hindley-Milner类型推导算法的经典论文。本文档§3的类型推导算法基于此论文。

3. [WrightFelleisen1994] Wright, A. K., & Felleisen, M. (1994). "A Syntactic Approach to Type Soundness". *Information and Computation*, 115(1): 38-94.
   - 类型安全证明的语法方法。本文档§4的类型安全证明参考此论文。

4. [Tarjan1983] Tarjan, R. E. (1983). *Data Structures and Network Algorithms*. SIAM.
   - 数据结构与算法分析的经典教材。本文档§2的算法类型定义参考此书。

**标准教材 / Standard Textbooks**:

1. [Cormen2022] Cormen, T. H., Leiserson, C. E., Rivest, R. L., & Stein, C. (2022). *Introduction to Algorithms* (4th Edition). MIT Press. ISBN: 978-0262046305.
   - 算法导论的经典教材。本文档§2-§3的算法复杂度类型参考此书。

2. [Pierce2002] Pierce, B. C. (2002). *Types and Programming Languages*. MIT Press.
   - 类型与程序设计语言的标准教材。本文档§2-§5的内容参考此书。

3. [Harper2016] Harper, R. (2016). *Practical Foundations for Programming Languages* (2nd Edition). Cambridge University Press.
   - 程序设计语言的实用基础。本文档§4-§5的类型安全理论参考此书。

**类型推导与统一 / Type Inference and Unification**:

1. [Wand1987] Wand, M. (1987). "A Simple Algorithm and Proof for Type Inference". *Fundamenta Informaticae*, 10: 115-122.
   - 类型推导的简单算法和证明。

**2024-2025 最新研究 / Recent Research (2024-2025)**:

1. [LogicalTypeSoundness2024] Jung, R., et al. (2024). "A Logical Approach to Type Soundness". *Journal of the ACM*. DOI: 10.1145/3676954.
   - Iris 框架下的语义类型可靠性证明，为算法类型安全提供新方法论。

2. [Lean4802024] The Lean Development Team. (2024). "Lean 4.8.0 Release Notes". <https://github.com/leanprover/lean4/releases/tag/v4.8.0>
   - Lean 4.8.0 在 defeq/WHNF 与类型系统实现层面的改进，直接影响算法类型系统的工程实践。

3. [MartinLof2024] Adjedj, A., et al. (2024). "Martin-Löf à la Coq". *CPP 2024*.
   - Martin-Löf 类型论元理论的 Coq 机械化，为从简单类型到依赖算法类型的形式化迁移奠定基础。
...
2. **Wikipedia - Type Theory**: <https://en.wikipedia.org/wiki/Type_theory>
   - 类型论的Wikipedia条目，介绍算法类型理论在类型理论中的发展（截至2025年1月11日）。

3. **Wikipedia - Type System**: <https://en.wikipedia.org/wiki/Type_system>
   - 类型系统的Wikipedia条目，包含类型推导和类型安全等核心概念（截至2025年1月11日）。
