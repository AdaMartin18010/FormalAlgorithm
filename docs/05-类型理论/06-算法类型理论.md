...
[1] 简单类型论基础：参见 `05-类型理论/01-简单类型论.md`。
[2] 依赖类型系统：参见 `05-类型理论/02-依赖类型论.md`。
[3] 类型系统与证明系统：参见 `03-形式化证明/01-证明系统.md`。
[4] 算法复杂度分析：参见 `04-算法复杂度/` 相关文档。
...
- [5.6 算法类型理论 / Algorithm Type Theory](#56-算法类型理论--algorithm-type-theory)
...
- [基本概念 / Basic Concepts](#基本概念--basic-concepts)
  - [算法类型定义 / Definition of Algorithm Types](#算法类型定义--definition-of-algorithm-types)
  - [算法类型分类 / Classification of Algorithm Types](#算法类型分类--classification-of-algorithm-types)
- [算法类型系统 / Algorithm Type System](#算法类型系统--algorithm-type-system)
  - [基本类型 / Basic Types](#基本类型--basic-types)
  - [类型环境 / Type Environment](#类型环境--type-environment)
  - [类型推导规则 / Type Inference Rules](#类型推导规则--type-inference-rules)
- [算法类型推导 / Algorithm Type Inference](#算法类型推导--algorithm-type-inference)
  - [统一算法 / Unification Algorithm](#统一算法--unification-algorithm)
  - [类型推导算法 / Type Inference Algorithm](#类型推导算法--type-inference-algorithm)
- [算法类型安全 / Algorithm Type Safety](#算法类型安全--algorithm-type-safety)
  - [类型安全定义 / Type Safety Definition](#类型安全定义--type-safety-definition)
  - [类型安全证明 / Type Safety Proofs](#类型安全证明--type-safety-proofs)
- [算法类型优化 / Algorithm Type Optimization](#算法类型优化--algorithm-type-optimization)
  - [类型特化 / Type Specialization](#类型特化--type-specialization)
  - [类型级优化 / Type-Level Optimization](#类型级优化--type-level-optimization)
...
- [实现示例 / Implementation Examples](#实现示例--implementation-examples)
  - [排序算法类型 / Sorting Algorithm Types](#排序算法类型--sorting-algorithm-types)
  - [搜索算法类型 / Search Algorithm Types](#搜索算法类型--search-algorithm-types)
- [未来发展方向 / Future Directions](#未来发展方向--future-directions)
  - [依赖算法类型 / Dependent Algorithm Types](#依赖算法类型--dependent-algorithm-types)
  - [同伦算法类型 / Homotopy Algorithm Types](#同伦算法类型--homotopy-algorithm-types)
- [参考文献](#参考文献)
- [知识导航](#知识导航)
- [学习目标](#学习目标)
...
算法类型理论结合了类型系统与算法分析，为算法设计和验证提供类型层面的保证 [Pierce2002, Cormen2022]。

### 算法类型定义 / Definition of Algorithm Types

算法类型理论是研究算法及其计算过程的类型系统的理论 [Tarjan1983]。
...
算法类型系统的设计借鉴了Hindley-Milner类型系统 [DamasMilner1982] 和系统F [Girard1972]。
...
类型推导算法基于Hindley-Milner算法 [DamasMilner1982, Wand1987]。

### 统一算法 / Unification Algorithm

统一算法由Robinson首先提出 [Robinson1965]，是类型推导的核心组件。
...
类型安全是类型系统的核心性质，包含进展性和保持性两个基本定理 [WrightFelleisen1994, Pierce2002]。
...
类型安全证明通常使用结构性操作语义和进展-保持方法 [WrightFelleisen1994]。
...
类型级优化利用类型信息指导程序优化 [Jones2000, Harper2016]。

### 类型特化 / Type Specialization

类型特化通过将多态类型实例化为具体类型来获得性能提升 [Jones2000]。
...
[1] Reynolds, J. C. "Towards a Theory of Type Structure". In *Programming Symposium*, 408-425. Springer., 1974.
[2] Reynolds关于类型结构的早期工作，类型理论的重要文献。本文档的理论基础参考此论文。

[3] Girard, J. Y., Lafont, Y., & Taylor, P. *Proofs and Types*. Cambridge University Press., 1989.
[4] 证明论与类型论的经典教材，深入介绍Curry-Howard同构。

[5] Wadler, P. "Propositions as Types". *Communications of the ACM*, 58(12): 75-84. DOI: 10.1145/2699407., 2015.
[6] Wadler对Curry-Howard同构的现代综述，获ACM SIGPLAN奖。本文档的算法类型理论基于此论文。
...
[9] Harper, R. *Practical Foundations for Programming Languages* (2nd Edition). Cambridge University Press., 2016.
[10] 程序设计语言的实用基础，系统介绍类型系统。
...
[12] 同伦类型论的标准教材，类型论的现代发展。

[13] Voevodsky, V. "The Origins and Motivations of Univalent Foundations". IAS., 2014.
[14] Voevodsky关于统一基础的动机，Fields奖得主的工作。

**经典奠基文献 / Classic Foundational Literature**:

[15] Robinson, J. A. "A Machine-Oriented Logic Based on the Resolution Principle". *Journal of the ACM*, 12(1): 23-41. DOI: 10.1145/321250.321253., 1965.
[16] 统一算法的经典论文，类型推导的核心组件。本文档§3.1的统一算法基于此论文。

[17] Damas, L., & Milner, R. "Principal Type-Schemes for Functional Programs". *POPL '82*, 207-212. DOI: 10.1145/582153.582176., 1982.
[18] Hindley-Milner类型推导算法的经典论文。本文档§3的类型推导算法基于此论文。

[19] Wright, A. K., & Felleisen, M. "A Syntactic Approach to Type Soundness". *Information and Computation*, 115(1): 38-94., 1994.
[20] 类型安全证明的语法方法。本文档§4的类型安全证明参考此论文。

[21] Tarjan, R. E. *Data Structures and Network Algorithms*. SIAM., 1983.
[22] 数据结构与算法分析的经典教材。本文档§2的算法类型定义参考此书。

**标准教材 / Standard Textbooks**:

[23] Cormen, T. H., Leiserson, C. E., Rivest, R. L., & Stein, C. *Introduction to Algorithms* (4th Edition). MIT Press. ISBN: 978-0262046305., 2022.
[24] 算法导论的经典教材。本文档§2-§3的算法复杂度类型参考此书。

[25] Pierce, B. C. *Types and Programming Languages*. MIT Press., 2002.
[26] 类型与程序设计语言的标准教材。本文档§2-§5的内容参考此书。

[27] Harper, R. *Practical Foundations for Programming Languages* (2nd Edition). Cambridge University Press., 2016.
[28] 程序设计语言的实用基础。本文档§4-§5的类型安全理论参考此书。

**类型推导与统一 / Type Inference and Unification**:

[29] Wand, M. "A Simple Algorithm and Proof for Type Inference". *Fundamenta Informaticae*, 10: 115-122., 1987.
[30] 类型推导的简单算法和证明。

**2024-2025 最新研究 / Recent Research (2024-2025)**:

[31] Jung, R., et al. "A Logical Approach to Type Soundness". *Journal of the ACM*. DOI: 10.1145/3676954., 2024.
[32] Iris 框架下的语义类型可靠性证明，为算法类型安全提供新方法论。

[33] The Lean Development Team. "Lean 4.8.0 Release Notes". <https://github.com/leanprover/lean4/releases/tag/v4.8.0>, 2024.
[34] Lean 4.8.0 在 defeq/WHNF 与类型系统实现层面的改进，直接影响算法类型系统的工程实践。

[35] Adjedj, A., et al. "Martin-Löf à la Coq". *CPP 2024*., 2024.
[36] Martin-Löf 类型论元理论的 Coq 机械化，为从简单类型到依赖算法类型的形式化迁移奠定基础。

**在线资源 / Online Resources**:

[37] Types and Programming Languages - Companion Website: <https://www.cis.upenn.edu/~bcpierce/tapl/>
[38] TAPL教材的配套资源。

[39] Wikipedia - Type Theory: <https://en.wikipedia.org/wiki/Type_theory>
[40] 类型论的Wikipedia条目，介绍算法类型理论在类型理论中的发展（截至2025年1月11日）。

[41] Wikipedia - Type System: <https://en.wikipedia.org/wiki/Type_system>
[42] 类型系统的Wikipedia条目，包含类型推导和类型安全等核心概念（截至2025年1月11日）。

[43] nLab - Type Theory: <https://ncatlab.org/nlab/show/type+theory>
[44] 类型论的范畴论视角。
