...
[3] 类型系统实现：参见 `05-类型理论/04-类型系统.md`。
[4] λ演算基础：参见 `07-计算模型/02-λ演算.md`。
...
高阶类型系统允许类型作为一等公民，支持类型参数化和类型级编程 [Girard1972, Reynolds1974, Barendregt1992]。

### 高阶类型定义 / Definition of Higher-Order Types

高阶类型系统是支持类型作为一等公民的类型系统，允许类型参数化、类型构造器和类型级编程 [Pierce2002]。
...
类型构造器是类型系统的核心，从基本类型构建复合类型 [Reynolds1974, Pierce2002]。
...
高阶类型推导扩展了Hindley-Milner算法以支持高阶类型 [DamasMilner1982, Jones1999]。
...
高阶类型安全确保类型构造和应用保持类型正确性 [Harper2016, Pierce2002]。
...
高阶类型系统在函数式编程和证明助手中广泛应用 [Harper2016, Jones2000]。
...
[2] System F的开创性工作，高阶类型系统的奠基之作。本文档的理论基础。

[3] Reynolds, J. C. "Types, Abstraction, and Parametric Polymorphism". *Information Processing '83*, 513-523., 1983.
[4] 参数多态和类型抽象的理论基础，高阶类型系统的重要文献。本文档的参数多态内容基于此论文。

[5] Wadler, P., & Blott, S. "How to Make Ad-hoc Polymorphism Less Ad Hoc". *POPL '89*, 60-76. DOI: 10.1145/75277.75283., 1989.
[6] 类型类（type classes）的开创性论文，Haskell类型系统的基础。本文档的类型类内容基于此论文。

**标准教材 / Standard Textbooks**:

[7] Pierce, B. C. *Types and Programming Languages*. MIT Press., 2002.
[8] 类型与程序设计语言的标准教材，全面介绍高阶类型系统。

[9] Harper, R. *Practical Foundations for Programming Languages* (2nd Edition). Cambridge University Press., 2016.
[10] 程序设计语言的实用基础，系统介绍高阶类型系统。

[11] Girard, J. Y., Lafont, Y., & Taylor, P. *Proofs and Types*. Cambridge University Press., 1989.
[12] 证明论与类型论的经典教材。

**类型论与逻辑 / Type Theory and Logic**:

[13] Wadler, P. "Propositions as Types". *Communications of the ACM*, 58(12): 75-84. DOI: 10.1145/2699407., 2015.
[14] Wadler对Curry-Howard同构的现代综述，获ACM SIGPLAN奖。

[15] Univalent Foundations Program. *Homotopy Type Theory: Univalent Foundations of Mathematics*. Institute for Advanced Study., 2013.
[16] 同伦类型论的标准教材，高阶类型系统的现代发展。

**经典奠基文献 / Classic Foundational Literature**:

[17] Girard, J. Y. "Interprétation fonctionnelle et élimination des coupures de l'arithmétique d'ordre supérieur". PhD thesis, Université Paris VII., 1972.
[18] System F的发现，多态λ演算的开创性工作。本文档§1的高阶类型定义基于此论文。

[19] Reynolds, J. C. "Towards a Theory of Type Structure". *Programming Symposium*, 408-425. Springer., 1974.
[20] 类型结构理论的先驱工作，多态类型系统的理论基础。本文档§2的类型构造器基于此论文。

[21] Barendregt, H. P. "Lambda Calculi with Types". *Handbook of Logic in Computer Science*, Vol. 2, 117-309., 1992.
[22] 类型化λ演算的综合手册。本文档§1的高阶类型分类参考此文。

**标准教材 / Standard Textbooks**:

[23] Pierce, B. C. *Types and Programming Languages*. MIT Press., 2002.
[24] 类型与程序设计语言的标准教材。本文档§1-§4的内容参考此书。

[25] Harper, R. *Practical Foundations for Programming Languages* (2nd Edition). Cambridge University Press., 2016.
[26] 程序设计语言的实用基础。本文档§4-§5的类型安全理论参考此书。

**类型系统与推导 / Type Systems and Inference**:

[27] Damas, L., & Milner, R. "Principal Type-Schemes for Functional Programs". *POPL '82*, 207-212., 1982.
[28] Hindley-Milner类型推导算法。

[29] Jones, S. P., Vytiniotis, D., Weirich, S., & Shields, M. "Practical Type Inference for Arbitrary-Rank Types". *Journal of Functional Programming*, 17(1): 1-82., 2007.
[30] 任意秩类型的实用类型推导。

**2024-2025 最新研究 / Recent Research (2024-2025)**:

[31] Poiret, J., et al. "All Your Base Are Belong to Us: Sort Polymorphism for Proof Assistants". *POPL'25*. DOI: 10.1145/3704912., 2025.
[32] 排序多态性及其元理论，支持跨排序的高阶类型定义复用。

[33] The Lean Development Team. "Lean 4.8.0 Release Notes". <https://github.com/leanprover/lean4/releases/tag/v4.8.0>, 2024.
[34] Lean 4.8.0 在高阶类型系统实现与类型推断方面的改进。

[35] Jung, R., et al. "A Logical Approach to Type Soundness". *Journal of the ACM*. DOI: 10.1145/3676954., 2024.
[36] Iris 框架下的语义类型可靠性证明，为高阶类型安全提供新方法论。

**在线资源 / Online Resources**:

[37] Wikipedia - System F: <https://en.wikipedia.org/wiki/System_F>
[38] System F的Wikipedia条目，高阶类型系统的基础（截至2025年1月11日）。

[39] Wikipedia - Type Theory: <https://en.wikipedia.org/wiki/Type_theory>
[40] 类型论的Wikipedia条目，介绍高阶类型系统在类型理论中的地位（截至2025年1月11日）。

[41] Wikipedia - Parametric Polymorphism: <https://en.wikipedia.org/wiki/Parametric_polymorphism>
[42] 参数多态的Wikipedia条目，高阶类型系统的核心概念（截至2025年1月11日）。

[43] Haskell Wiki - Type Class: <https://wiki.haskell.org/Type_class>
[44] 类型类的实用介绍。

[45] nLab - Higher-Order Type Theory: <https://ncatlab.org/nlab/show/higher-order+type+theory>
[46] 高阶类型论的范畴论视角。
