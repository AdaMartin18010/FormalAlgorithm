...
1. [Girard1972] Girard, J. Y. (1972). "Interprétation fonctionnelle et élimination des coupures de l'arithmétique d'ordre supérieur". PhD thesis, Université Paris VII.
   - System F的发现，多态λ演算的开创性工作。本文档§1的高阶类型定义基于此论文。

2. [Reynolds1974] Reynolds, J. C. (1974). "Towards a Theory of Type Structure". *Programming Symposium*, 408-425. Springer.
   - 类型结构理论的先驱工作，多态类型系统的理论基础。本文档§2的类型构造器基于此论文。

3. [Barendregt1992] Barendregt, H. P. (1992). "Lambda Calculi with Types". *Handbook of Logic in Computer Science*, Vol. 2, 117-309.
   - 类型化λ演算的综合手册。本文档§1的高阶类型分类参考此文。

**标准教材 / Standard Textbooks**:

1. [Pierce2002] Pierce, B. C. (2002). *Types and Programming Languages*. MIT Press.
   - 类型与程序设计语言的标准教材。本文档§1-§4的内容参考此书。

2. [Harper2016] Harper, R. (2016). *Practical Foundations for Programming Languages* (2nd Edition). Cambridge University Press.
   - 程序设计语言的实用基础。本文档§4-§5的类型安全理论参考此书。

**类型系统与推导 / Type Systems and Inference**:

1. [DamasMilner1982] Damas, L., & Milner, R. (1982). "Principal Type-Schemes for Functional Programs". *POPL '82*, 207-212.
   - Hindley-Milner类型推导算法。

2. [Jones1999] Jones, S. P., Vytiniotis, D., Weirich, S., & Shields, M. (2007). "Practical Type Inference for Arbitrary-Rank Types". *Journal of Functional Programming*, 17(1): 1-82.
   - 任意秩类型的实用类型推导。

**2024-2025 最新研究 / Recent Research (2024-2025)**:

1. [CoqSortPolymorphism2025] Poiret, J., et al. (2025). "All Your Base Are Belong to Us: Sort Polymorphism for Proof Assistants". *POPL'25*. DOI: 10.1145/3704912.
   - 排序多态性及其元理论，支持跨排序的高阶类型定义复用。
...
2. [Lean4802024] The Lean Development Team. (2024). "Lean 4.8.0 Release Notes". <https://github.com/leanprover/lean4/releases/tag/v4.8.0>
   - Lean 4.8.0 在高阶类型系统实现与类型推断方面的改进。

3. [LogicalTypeSoundness2024] Jung, R., et al. (2024). "A Logical Approach to Type Soundness". *Journal of the ACM*. DOI: 10.1145/3676954.
   - Iris 框架下的语义类型可靠性证明，为高阶类型安全提供新方法论。

**在线资源 / Online Resources**:

1. **Wikipedia - System F**: <https://en.wikipedia.org/wiki/System_F>
   - System F的Wikipedia条目，高阶类型系统的基础（截至2025年1月11日）。

2. **Wikipedia - Type Theory**: <https://en.wikipedia.org/wiki/Type_theory>
   - 类型论的Wikipedia条目，介绍高阶类型系统在类型理论中的地位（截至2025年1月11日）。

3. **Wikipedia - Parametric Polymorphism**: <https://en.wikipedia.org/wiki/Parametric_polymorphism>
   - 参数多态的Wikipedia条目，高阶类型系统的核心概念（截至2025年1月11日）。
