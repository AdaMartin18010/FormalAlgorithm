...
反证法是通过假设命题的否定为真，然后推导出矛盾来证明原命题的方法；在经典逻辑中与排中律等价，在直觉主义逻辑中受限（见 [VanDalen2013]）。

**Definition 1.1.1** (Proof by Contradiction) [Enderton2001][Mendelson2015]
Proof by contradiction is a method of proving a proposition by assuming its negation is true and then deriving a contradiction; it is equivalent to the law of excluded middle in classical logic and is restricted in intuitionistic logic (see [VanDalen2013]).
...
**解释与直观**：反证法通过假设 $\neg P$ 推出矛盾 $\bot$ 得到 $P$，依赖排中律；在直觉主义逻辑中不可用于任意命题，但与双重否定消去等价。
...
**概念关系**：反证法 —depends_on— 经典逻辑/排中律；反证法 —equivalent_to— 归谬（reductio）；与构造性证明 —对比— 对存在命题不提供见证。
...
| 方法 | 假设 | 结论方式 | 直觉主义 |
|------|------|----------|----------|
| 反证法 | $\neg P$ | $\bot \Rightarrow P$ | 不普遍有效 |
| 直接证明 | 公理/前提 | 推理链到 $P$ | 有效 |
| 构造性 | — | 给出见证 | 有效 |

**公理定理推理证明决策树**：定义 2.1.1（反证法）→ 假设步、推导步、结论步；经典逻辑中 $\neg\neg P \to P$ 与反证法等价；可靠性依赖爆炸原理与排中律。

**应用决策建模树**：证唯一性/非存在（「至多一个」「不存在」）→ 反证法常用；需可计算内容或 Coq/Agda 形式化 → 优先直接或构造性证明；经典数学叙述 → 反证法可用（见 §2）。
...
**定义 3.1.1** (排中律 / Law of Excluded Middle) [Enderton2001][Troelstra2000]
...
**定义 3.2.1** (否定引入规则 / Negation Introduction Rule) [Prawitz1965][Troelstra2000]
否定引入规则是自然演绎系统中的规则：
...
**定义 3.3.1** (矛盾消除规则 / Contradiction Elimination Rule) [Enderton2001][Mendelson2015]
矛盾消除规则允许从矛盾推导出任意命题：
...
**定义 6.2.1** (直觉主义逻辑中的反证法 / Proof by Contradiction in Intuitionistic Logic) [VanDalen2013][TroelstraVanDalen1988]
在直觉主义逻辑中，反证法的地位：
...
**定理 6.3.1** (经典逻辑中的反证法 / Proof by Contradiction in Classical Logic) [Enderton2001][Troelstra2000]
在经典逻辑中，反证法等价于排中律。
...
3. Wikipedia - Proof by Contradiction: <https://en.wikipedia.org/wiki/Proof_by_contradiction>
   - 反证法的Wikipedia条目，包含经典逻辑中的反证法原理和应用（截至2025年1月11日）。

4. Wikipedia - Reductio ad Absurdum: <https://en.wikipedia.org/wiki/Reductio_ad_absurdum>
   - 归谬法的Wikipedia条目，详细介绍反证法的逻辑基础（截至2025年1月11日）。
