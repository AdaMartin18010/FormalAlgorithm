...
- [1.4 内容补充与思维表征 / Content Supplement and Thinking Representation](#14-内容补充与思维表征--content-supplement-and-thinking-representation)
  - [1.5 BHK 解释与构造性语义 / BHK Interpretation](#15-bhk-解释与构造性语义--bhk-interpretation)
...
- [5. 构造性数学 (Constructive Mathematics)](#5-构造性数学-constructive-mathematics)
  ...
  - [5.5 构造性分析与经典分析的对比 / Constructive vs Classical Analysis](#55-构造性分析与经典分析的对比--constructive-vs-classical-analysis)
  - [5.4 可实现性理论 / Realizability Theory](#54-可实现性理论--realizability-theory)
...
**定义 1.0.1** (构造性证明 / Constructive Proof) [Bishop1967][TroelstraVanDalen1988]
构造性证明是满足以下条件的证明：
...
3. **构造性蕴涵 / Constructive Implication**: 对于蕴涵命题 $A \r\r\\rightarrow B$，提供从 $A$ 到 $B$ 的构造性函数
   For implicative proposition $A \r\r\\rightarrow B$, provide constructive function from $A$ to $B$
...
**定理 1.0.1** (构造性证明存在性定理) [Beeson1985] 对于任何可构造的数学对象，存在构造性证明。
...
| $A \lor B$ | 一个变体 $ ext$\text{inl}(p)$ 或 $ ext$\text{inr}(q)$，其中 $p$ 是 $A$ 的证明或 $q$ 是 $B$ 的证明 |
| $A
\r\\rightarrow B$ | 一个构造性函数 $f$，将 $A$ 的任意证明 $p$ 映射为 $B$ 的证明 $f(p)$ |
| $\exists x \in X.\, P(x)$ | 一个二元组 $(a, p)$，其中 $a \in X$ 是见证，$p$ 是 $P(a)$ 的证明 |
| $orall x \in X.\, P(x)$ | 一个构造性函数 $f$，将每个 $a \in X$ 映射为 $P(a)$ 的证明 $f(a)$ |
| $
\neg A$ | 一个构造性函数 $f$，将 $A$ 的任意证明映射为矛盾（即 $f : A
\r\\rightarrow ot$） |

**定理 1.5.1** (BHK 与经典真值表的不可调和性) [1]
在 BHK 解释下，排中律 $A \lor
\neg A$ 不具有普遍有效性，因为对于任意命题 $A$，我们无法保证总能够构造出 $A$ 的证明或 $A
\r\\rightarrow ot$ 的证明。
...
**定义 5.1.1** (直觉主义数学 / Intuitionistic Mathematics) [Heyting1956][TroelstraVanDalen1988]
直觉主义数学强调构造性证明，拒绝排中律。
...
**定义 5.2.1** (构造性分析 / Constructive Analysis) [Bishop1967][BridgesRichman1987]
构造性分析是构造性数学在分析学中的应用。
...
**定义 5.3.1** (构造性代数 / Constructive Algebra) [MinesRichmanRuitenburg1988]
构造性代数是构造性数学在代数学中的应用。
...
- \realizes A \lor B$ 当且仅当 _0 = 0$ 且 _1 \realizes A$，或 _0 = 1$ 且_1 \realizes B$；
...
5. [Heyting1956] Heyting, A. (1956). *Intuitionism: An Introduction* (2nd Edition). North-Holland.
   - 直觉主义数学的经典入门书，阐述了Brouwer-Heyting直觉主义哲学的核心观点。本文档§5.1的直觉主义数学参考此书。

6. [MinesRichmanRuitenburg1988] Mines, R., Richman, F., & Ruitenburg, W. (1988). *A Course in Constructive Algebra*. Springer. ISBN: 978-0387966403.
   - 构造性代数的系统教材。本文档§5.3的构造性代数参考此书。

**类型论与构造性证明 / Type Theory and Constructive Proofs**:

1. [MartinLof1984] Martin-Löf, P. (1984). *Intuitionistic Type Theory*. Bibliopolis. ISBN: 978-8870881054.
   - 直觉主义类型论的系统阐述，构造性证明的类型论基础。本文档§1的类型论联系参考此书。
...
1. [Coq] The Coq Development Team. (2024). *The Coq Proof Assistant*. Retrieved from <https://coq.inria.fr/> [Accessed: 2025-04-15].
    - 基于构造演算的证明助手，实现构造性证明。本文档§7.3的应用参考此工具。

2. [Agda] Norell, U. (2007). "Towards a Practical Programming Language Based on Dependent Type Theory". PhD thesis, Chalmers University of Technology.
    - Agda语言，依赖类型的构造性证明系统。本文档§7.3的应用参考此工具。

**在线资源 / Online Resources**:

1. nLab contributors. (2024). "Constructive Mathematics". nLab. Retrieved from <https://ncatlab.org/nlab/show/constructive+mathematics> [Accessed: 2025-04-15].
   - 构造性数学的范畴论视角。

2. Wikipedia contributors. (2025). "Constructive Proof". Wikipedia, The Free Encyclopedia. Retrieved from <https://en.wikipedia.org/wiki/Constructive_proof> [Accessed: 2025-04-15].
   - 构造性证明的Wikipedia条目，包含最新定义和示例。

3. Wikipedia contributors. (2025). "Constructive Mathematics". Wikipedia, The Free Encyclopedia. Retrieved from <https://en.wikipedia.org/wiki/Constructive_mathematics> [Accessed: 2025-04-15].
   - 构造性数学的Wikipedia条目，涵盖Bishop构造性分析等主要流派。

4. Wikipedia contributors. (2025). "Intuitionistic Logic". Wikipedia, The Free Encyclopedia. Retrieved from <https://en.wikipedia.org/wiki/Intuitionistic_logic> [Accessed: 2025-04-15].
   - 直觉主义逻辑的Wikipedia条目，包含构造性证明的逻辑基础。
