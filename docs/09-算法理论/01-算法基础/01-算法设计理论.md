...
算法设计与分析可与 **MIT 6.006/6.046**、**CMU 15-451 Algorithm Design and Analysis**、**Stanford CS 161**、**Berkeley CS 170** 等课程对标。课程与模块映射及建议阅读顺序见 [国际课程对标表](../../国际课程对标表.md)。
...
- [1. 基本概念 / Basic Concepts](#1-基本概念--basic-concepts)
  ...
  - [1.4 内容补充与思维表征 / Content Supplement and Thinking Representation](#14-内容补充与思维表征--content-supplement-and-thinking-representation)
    ...
    - [多维矩阵：设计范式与证明方法对比 / Multi-Dimensional Comparison](#多维矩阵设计范式与证明方法对比--multi-dimensional-comparison)
    - [决策树：问题结构到范式与正确性方法选择 / Decision Tree](#决策树问题结构到范式与正确性方法选择--decision-tree)
...
算法设计理论将算法形式化为 $(I,O,P)$，强调有限性、确定性、可执行性；分治、动态规划、贪心等范式与循环不变式、归纳证明构成正确性与复杂度分析的基础。与 01-基础理论、04-算法复杂度、07-计算模型、09-01 各专题、03-形式化证明衔接；§1–§5 形成完整表征。

#### 概念属性表 / Concept Attribute Table

| 属性名 | 类型/范围 | 含义 | 备注 |
|--------|-----------|------|------|
| 算法 $A=(I,O,P)$、有限性/确定性/可执行性 | 基本概念 | 定义 1.1.1、§1.2 | 与 04、07 对照 |
| 伪代码/流程图/程序、分治/DP/贪心、循环不变式/归纳 | 表示/范式/证明 | §1.3、§2、§3 | 子问题重叠性、最优子结构、贪心选择 |
| 分治/DP/贪心、循环不变式/归纳/形式化验证 | 对比 | §2、§3 | 多维矩阵 |

#### 概念关系 / Concept Relations

| 源概念 | 目标概念 | 关系类型 | 说明 |
|--------|----------|----------|------|
| 算法设计理论 | 01-基础理论、04-算法复杂度、07-计算模型 | depends_on | 基础与复杂度、计算模型 |
| 算法设计理论 | 09-01 各专题、03-形式化证明 | relates_to | 设计范式应用、正确性证明 |
...
定理 1.3.1–1.3.3 表示等价与终止性；§3 循环不变式与归纳；§4 时间空间复杂度；与 03 形式化证明衔接。
...
graph TD
  Design[算法设计理论]
  Design --> Def[算法定义与表示]
  Design --> Paradigm[设计范式]
  Design --> Correct[正确性证明]
  Design --> Complex[复杂度分析]
  Design --> Impl[实现与验证 §5]
  Paradigm --> DC[分治/DP/贪心]
```

#### 多维矩阵：设计范式与证明方法对比 / Multi-Dimensional Comparison

| 概念/方法 | 子问题重叠性/最优子结构/贪心选择 | 证明难度与自动化 | 备注 |
|-----------|----------------------------------|------------------|------|
| 分治/DP/贪心 | §2 | §3 | — |
| 循环不变式/归纳/形式化验证 | — | §3 | — |

#### 决策树：问题结构到范式与正确性方法选择 / Decision Tree

```mermaid
flowchart TD
  Start([问题结构])
  Start --> Struct{重叠子问题/最优子结构/贪心选择?}
  Struct --> Paradigm[选分治/DP/贪心 §2]
  Start --> Correct{正确性工具?}
  Correct --> Tool[不变式/归纳/形式化 §3]
  Paradigm --> Impl[§5]
  Tool --> Impl
```
...
flowchart TD
  Need([应用需求])
  Need --> Paradigm[选设计范式 分治/DP/贪心 §2]
  Need --> Correct[选正确性方法 不变式/归纳/验证 §3]
  Need --> Anal[选分析指标 时间/空间 §4]
  Paradigm --> Impl[§5]
  Correct --> Impl
  Anal --> Impl
```
...
```mermaid
mindmap
  root((算法设计范式<br/>Algorithm Design Paradigms))
    分治法
      递归分解
      子问题独立
      Master定理
...
> 📚 **概念引用**：关于 **动态规划** 的完整定义，详见 [`docs/知识图谱/concept_nodes.yaml`](../../知识图谱/concept_nodes.yaml) 中 `dp` 的定义，或参见 [`docs/09-算法理论/01-算法基础/06-动态规划理论.md`](../../09-算法理论/01-算法基础/06-动态规划理论.md)。
