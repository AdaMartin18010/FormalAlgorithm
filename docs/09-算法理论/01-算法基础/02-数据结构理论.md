---
title: 9.1.2 数据结构理论 / Data Structure Theory
version: 1.2
status: maintained
...
数据结构可与 **MIT 6.006**、**CMU 15-451**、**Stanford CS 161**、**Berkeley CS 170** 等课程对标。课程与模块映射见 [国际课程对标表](../../国际课程对标表.md)。
...
- [1. 基本概念 / Basic Concepts](#1-基本概念--basic-concepts)
  ...
  - [1.4 内容补充与思维表征 / Content Supplement and Thinking Representation](#14-内容补充与思维表征--content-supplement-and-thinking-representation)
    ...
    - [多维矩阵：结构与存储对比 / Multi-Dimensional Comparison](#多维矩阵结构与存储对比--multi-dimensional-comparison)
    - [决策树：访问模式到结构选择 / Decision Tree](#决策树访问模式到结构选择--decision-tree)
...
- [X. 正确性证明 / Correctness Proofs](#x-正确性证明--correctness-proofs)
  - [X.1 循环不变式 / Loop Invariants](#x1-循环不变式--loop-invariants)
  - [X.2 终止性证明 / Termination Proofs](#x2-终止性证明--termination-proofs)
  - [X.3 部分正确性与完全正确性 / Partial and Total Correctness](#x3-部分正确性与完全正确性--partial-and-total-correctness)
- [参考文献（补充）](#参考文献补充)
- [参考文献](#参考文献)
- [知识导航](#知识导航)
- [学习目标](#学习目标)
...
数据结构理论用 ADT 与逻辑/存储分类统一线性、树、图、散列结构；操作复杂度与摊还分析支撑算法选型。与 09-01 各算法专题紧密联动；§1.1–§1.3、§2–§6 形成完整表征。

#### 概念属性表 / Concept Attribute Table

| 属性名 | 类型/范围 | 含义 | 备注 |
|--------|-----------|------|------|
| 数据结构、定义与 ADT、线性/树/图/散列、顺序/链式/索引/散列存储 | 基本概念/分类 | §1.1–§1.3 | 与 04、09-01-01、23 对照 |
| 各结构操作与复杂度、摊还分析 | 操作/复杂度 | §2–§5 | 逻辑结构、主要操作、摊还/最坏复杂度 |
| 线性/树/图/散列、顺序/链式/散列存储 | 对比 | §2–§5 | 多维矩阵 |

#### 概念关系 / Concept Relations

| 源概念 | 目标概念 | 关系类型 | 说明 |
|--------|----------|----------|------|
| 数据结构理论 | 01-基础理论、04-算法复杂度 | depends_on | 基础与复杂度 |
| 数据结构理论 | 09-01-01 算法设计、23-数据结构多维分析 | relates_to | 数据结构支撑算法、多维对比 |
...
定义 1.3.1–1.3.2 分类；各节操作复杂度与摊还分析见 §2–§5；与 23-多维分析 对照。
...
#### 多维矩阵：结构与存储对比 / Multi-Dimensional Comparison

| 概念/结构 | 逻辑结构 | 主要操作 | 摊还/最坏复杂度 | 备注 |
|-----------|----------|----------|------------------|------|
| 线性/树/图/散列 | §2–§5 | §2–§5 | §2–§5 | — |
| 顺序/链式/散列存储 | 存取方式 | 适用场景 | §2–§5 | — |

#### 决策树：访问模式到结构选择 / Decision Tree

```mermaid
flowchart TD
  Start([访问模式/插入删除频率])
  Start --> Access{随机/顺序/键查找?}
  Access --> Struct[选线性/树/散列 §2–§5]
  Start --> Freq{插入删除频率?}
  Freq --> Impl[选链式/平衡树/散列 §2–§5]
  Struct --> Impl
```
...
flowchart TD
  Need([应用需求])
  Need --> App{问题需求?}
  App -->|栈/队列/优先队列/图/键值| Struct[选结构 §2–§5]
  Struct --> Impl[§6 与 23]
```
