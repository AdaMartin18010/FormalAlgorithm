...
graph LR
  Ver[验证公设 §2]
  Hoare[Hoare系统 §1.2]
  Complete[完备性与可靠性 定理1.2.1-1.2.2]
  WP[wp与循环不变量 §1.3-§1.4]
  Ver --> Hoare
  Hoare --> Complete
  Complete --> WP
...
flowchart TD
  Need([需求：排序/图算法等验证])
  Need --> PrePost[前置后置/循环不变量/归纳 §7]
  PrePost --> Lean[与08-Lean、03一致]
```
