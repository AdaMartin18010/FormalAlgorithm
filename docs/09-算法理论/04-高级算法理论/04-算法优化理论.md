...
算法优化理论与高级优化可与 **MIT 6.046**、**CMU 15-451**、**Stanford CS 161**、**Berkeley CS 170** 等课程对标。课程与模块映射见 [国际课程对标表](../../../国际课程对标表.md)。
...
- [2. 基本概念 / Basic Concepts](#2-基本概念--basic-concepts)
  ...
  - [2.3 内容补充与思维表征 / Content Supplement and Thinking Representation](#23-内容补充与思维表征--content-supplement-and-thinking-representation)
    ...
    - [多维矩阵：优化策略概念对比 / Multi-Dimensional Comparison](#多维矩阵优化策略概念对比--multi-dimensional-comparison)
    - [决策树：优化策略选择 / Decision Tree](#决策树优化策略选择--decision-tree)
...
**算法优化系统（定义 1.1.1）的动机**：将「算法—目标—策略—性能」统一为五元组，便于形式化讨论优化问题的可解性、收敛性与复杂度。直观上，$F$ 表示「在目标 $O$ 与策略 $S$ 下对算法 $a$ 做一步优化得到 $a'$」，$P$ 表示「算法好坏」的数值度量。

**与已有概念的联系**：算法优化系统特化了计算模型中的「程序—规约」关系（见 07-计算模型）；优化空间 $(\mathcal{X}, d, f)$ 与 09-算法理论中的搜索空间、目标函数一致；优化策略 $\sigma$ 对应迭代算法中的单步更新规则。

#### 概念属性表 / Concept Attribute Table

| 属性名 | 类型/范围 | 含义 | 备注 |
|--------|-----------|------|------|
| $A$ | 集合 | 候选算法集合 | 有限时定理 1.1.1 适用 |
| $O$ | 函数集合 | 优化目标（如最小化 $T(n)$） | 可多目标 |
| $S$ | 策略集合 | 如何从当前算法得到下一算法 | 如梯度步、邻域搜索 |
| $F$ | $A \times O \times S \to A$ | 单步优化映射 | 确定性或随机 |
| $P$ | $A \to \mathbb{R}^+$ | 性能度量（越小越优） | 连续时便于存在性证明 |
| $T(n)$ | $\mathbb{N} \to \mathbb{R}^+$ | 时间复杂度 | 与 §4.1 一致 |
| $S(n)$ | $\mathbb{N} \to \mathbb{R}^+$ | 空间复杂度 | 与 §4.1 一致 |

#### 概念关系 / Concept Relations

| 源概念 | 目标概念 | 关系类型 | 说明 |
|--------|----------|----------|------|
| 算法优化系统 | 优化问题 | specializes | 优化问题由系统 + 目标定义 |
| 优化空间 | 算法优化系统 | applies_to | 连续优化在 $(\mathcal{X}, d, f)$ 上实例化 |
| 优化策略 | 优化收敛理论 | depends_on | 收敛性针对策略序列 |
| 算法优化 | 算法工程 | applies_to | 见 02-算法工程理论 |
...
**定理 1.1.1（优化问题存在性）的非形式论证**：在有限个候选算法中选「性能最好」的一个，本质上是在有限集上取最小值；实数域上有限集必有最小元，故解存在。形式证明见 §1.1 证明段落；下面给出从定义到该定理的推理依赖图。
...
#### 多维矩阵：优化策略概念对比 / Multi-Dimensional Comparison

| 概念/策略 | 优化层次 | 时间复杂度影响 | 空间复杂度影响 | 适用场景 | 形式化难度 |
|-----------|----------|----------------|----------------|----------|------------|
| 算法级优化 | 算法 | 直接（如降阶） | 可能降低 | 通用 | 中（不变式） |
| 数据结构优化 | 数据结构 | 间接（访问模式） | 常为主要目标 | 内存敏感 | 中（抽象） |
| 并行化优化 | 执行模型 | 理论线性加速 | 常增加 | 多核/分布式 | 高（一致性） |
| 编译器优化 | 编译时 | 自动改写 | 自动改写 | 通用 | 高（语义保持） |
| 运行时优化 | 运行时 | 自适应 | 自适应 | JIT/热路径 | 高（动态） |

#### 决策树：优化策略选择 / Decision Tree

```mermaid
flowchart TD
  Start([需要算法优化])
  Start --> Goal{主要目标?}
  Goal -->|时间| Algo[算法级优化 §3.1]
  Goal -->|空间| DS[数据结构优化 §3.2]
  Goal -->|兼顾| Par{可并行?}
  Par -->|是| Parallel[并行化优化 §5.1]
  Par -->|否| Algo
  Goal -->|通用| Auto{可改源码?}
  Auto -->|否| Runtime[运行时优化 §6.2]
  Auto -->|是| Compiler[编译器优化 §6.1]
```
...
graph LR
  D111[定义 1.1.1 优化系统]
  D112[定义 1.1.2 优化问题]
  D112 --> T11[定理 1.1.1 存在性]
  Finite[有限性]
  Finite --> T11
  D211[定义 1.2.1 优化空间]
  D212[定义 1.2.2 局部最优]
  D211 --> D212
  D212 --> T21[定理 1.2.1 极值定理]
  EVT[极值定理]
  EVT --> T21
```
...
flowchart TD
  Need([需求：算法优化])
  Need --> Goal{优化目标?}
  Goal -->|时间| Sec31[算法级优化 §3.1]
  Goal -->|空间| Sec32[数据结构优化 §3.2]
  Goal -->|兼顾| Constraint{可并行?}
  Constraint -->|是| Sec51[并行化优化 §5.1]
  Constraint -->|否| Sec31
  Goal -->|可维护性| Sec61[编译器优化 §6.1]
```
