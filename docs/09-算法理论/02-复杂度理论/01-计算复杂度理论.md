...
计算复杂度理论可与 **MIT 18.404 Theory of Computation**、**CMU 15-251**、**Stanford CS 161**、**Berkeley CS 170** 等课程对标。课程名、syllabus 与模块映射见 [国际课程对标表](../../国际课程对标表.md)；建议阅读顺序：07-计算模型 → 04-算法复杂度 → 09-02-复杂度理论。
...
- [1. 基本概念 / Basic Concepts](#1-基本概念--basic-concepts)
  ...
  - [1.4 内容补充与思维表征 / Content Supplement and Thinking Representation](#14-内容补充与思维表征--content-supplement-and-thinking-representation)
    ...
    - [多维矩阵：复杂度类与下界方法对比 / Multi-Dimensional Comparison](#多维矩阵复杂度类与下界方法对比--multi-dimensional-comparison)
    - [决策树：问题类型与资源到下界方法选择 / Decision Tree](#决策树问题类型与资源到下界方法选择--decision-tree)
...
计算复杂度理论在给定计算模型（RAM/TM）下刻画问题与算法所需时间与空间资源；P、NP、PSPACE 等复杂度类构成层次，下界与归约是核心工具。与 04-算法复杂度、07-计算模型、09-01 算法基础衔接；§1–§5 形成完整表征。

#### 概念属性表 / Concept Attribute Table

| 属性名 | 类型/范围 | 含义 | 备注 |
|--------|-----------|------|------|
| RAM/TM、判定/优化、$E(A)=\alpha T_A+\beta S_A$ | 计算模型/问题分类/效率度量 | §1.1–§1.3 | 与 04、07 对照 |
| P/NP/PSPACE、下界、归约与完全性 | 复杂度类/下界/归约 | §4、§5 | 时间空间资源、确定性/非确定性 |
| P/NP/coNP/PSPACE、信息论/通信/电路 | 对比 | §4、§5 | 多维矩阵 |

#### 概念关系 / Concept Relations

| 源概念 | 目标概念 | 关系类型 | 说明 |
|--------|----------|----------|------|
| 计算复杂度理论 | 04-算法复杂度、07-计算模型 | depends_on | 复杂度与计算模型 |
| 计算复杂度理论 | 01-图灵机、09-01 算法基础 | relates_to | TM 为理论模型、算法分析基础 |
...
定义 1.1.1–1.3.1 形式化模型与效率；层次定理、完全性、下界见 §4–§5 及 04-复杂度类；下界证明结构见 §5。
...
graph TD
  CC[计算复杂度理论]
  CC --> TS[时间/空间复杂度]
  CC --> Class[复杂度类 P/NP/PSPACE]
  CC --> Bound[下界理论]
  CC --> Red[归约]
  CC --> Anal[算法分析、可计算性边界]
```

#### 多维矩阵：复杂度类与下界方法对比 / Multi-Dimensional Comparison

| 概念/类 | 时间空间资源 | 确定性/非确定性 | 下界方法 | 备注 |
|---------|--------------|-----------------|----------|------|
| P/NP/coNP/PSPACE | §4 | §4 | §5 | — |
| 归约、信息论/通信/电路 | 难度比较 | — | §5 | — |

#### 决策树：问题类型与资源到下界方法选择 / Decision Tree
...
flowchart TD
  Need([应用需求])
  Need --> Anal[算法分析 选 RAM/TM 与 O/Ω/Θ]
  Need --> Diff[问题难度 P vs NP、归约]
  Need --> Bound[下界证明 选信息论/通信/电路 见 04]
  Anal --> Impl[见 04]
  Diff --> Impl
  Bound --> Impl
```
