...
启发式与元启发式算法可与 **MIT 6.046**、**CMU 15-451**、**Stanford CS 161**、**Berkeley CS 170** 等算法课程及 AI 搜索专题对标。课程与模块映射见 [国际课程对标表](../../国际课程对标表.md)。
...
- [1. 基本概念 / Basic Concepts](#1-基本概念--basic-concepts)
  ...
  - [1.4 内容补充与思维表征 / Content Supplement and Thinking Representation](#14-内容补充与思维表征--content-supplement-and-thinking-representation)
    ...
    - [多维矩阵：启发式方法对比 / Multi-Dimensional Comparison](#多维矩阵启发式方法对比--multi-dimensional-comparison)
    - [决策树：启发式选型 / Decision Tree](#决策树启发式选型--decision-tree)
...
- [11. 深度主题：启发式分类学、近似保证、收敛理论与神经组合优化](#11-深度主题启发式分类学近似保证收敛理论与神经组合优化)
  - [11.1 启发式分类学](#111-启发式分类学)
  - [11.2 近似保证与概率界](#112-近似保证与概率界)
  - [11.3 收敛理论](#113-收敛理论)
  - [11.4 神经组合优化与学习优化](#114-神经组合优化与学习优化)
- [参考文献（补充）](#参考文献补充)
- [参考文献](#参考文献)
- [知识导航](#知识导航)
- [学习目标](#学习目标)
...
启发式算法用经验与启发式函数指导搜索，在可接受时间内求近似最优解。局部搜索、遗传算法、模拟退火、蚁群等形成不同搜索策略与适用场景的谱系；无全局最优保证，与 09-01-12 近似、09-01-16 生物算法衔接。

#### 概念属性表 / Concept Attribute Table

| 属性名 | 类型/范围 | 含义 | 备注 |
|--------|-----------|------|------|
| 启发式 $H(f,\Omega)$ | 形式化 | 定义 1.1.1 | 搜索空间 $S\subseteq\Omega$ |
| 局部搜索/群体智能/物理启发 | 策略类 | §1.2 | 邻域与跳出局部最优 |
| 遗传/退火/PSO/蚁群/禁忌 | 方法 | §2–§6 | 搜索策略、参数 |
| 收敛性、参数敏感性 | 分析 | §8 | 马尔可夫链、期望 |

#### 概念关系 / Concept Relations

| 源概念 | 目标概念 | 关系类型 | 说明 |
|--------|----------|----------|------|
| 启发式算法理论 | 09-03-01 算法优化、09-01-04 搜索 | depends_on | 优化与搜索 |
| 启发式算法理论 | 09-01-16 生物算法 | applies_to | 遗传/蚁群等重叠 |
| 启发式算法理论 | 04-复杂度、09-01-12 近似 | 概念对比 | 近似比与运行时间 |
...
定义 1.1.1 形式化 $H(f,\Omega)$；收敛性分析、参数敏感性见 §8；与精确算法、近似算法对比见 09-01-12。
...
graph TD
  ...
  Heur --> Swarm[群体智能]
  Heur --> Phys[物理启发]
  Heur --> Methods[遗传/退火/PSO/蚁群/禁忌]
```

#### 多维矩阵：启发式方法对比 / Multi-Dimensional Comparison

| 方法 | 搜索策略/邻域 | 收敛性 | 适用问题 |
|------|----------------|--------|----------|
| 遗传 | 选择/交叉/变异 | 马尔可夫链 | 组合/连续 |
| 模拟退火 | 邻域+接受概率 | 收敛到最优 | 组合/连续 |
| PSO | 位置/速度更新 | 收敛性依参数 | 连续 |
| 蚁群 | 信息素+路径 | 收敛性见文献 | 路径/调度 |
| 禁忌 | 邻域+禁忌表 | 局部收敛 | 组合 |

#### 决策树：启发式选型 / Decision Tree
