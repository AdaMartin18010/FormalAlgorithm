...
- [12.11 量子金融算法应用 / Quantum Financial Algorithm Applications](#1211-量子金融算法应用--quantum-financial-algorithm-applications)
  ...
  - [规约与模型在本领域的实例化 / Specification and Model Instantiation in Quantum Finance](#规约与模型在本领域的实例化--specification-and-model-instantiation-in-quantum-finance)
...
flowchart TD
  Start([量子金融场景])
  Start --> Need{主要需求?}
  Need -->|期权/衍生品定价| Opt[量子期权定价 §量子期权定价算法]
  Need -->|投资组合优化| Port[量子投资组合 §各节]
  Need -->|风险度量| Risk[量子风险度量 §各节]
  Need -->|模拟/积分| QMC[量子蒙特卡洛 §各节]
  Need -->|预测/因子| QML[量子ML金融 §各节]
  Opt --> QMC
  Port --> QUBO[QUBO/绝热 §各节]
```
...
flowchart TD
  Need([应用需求：量子金融])
  Need --> App{应用领域?}
  App -->|定价| Opt[量子期权定价 §实现示例]
  App -->|优化| Port[量子投资组合 §各节]
  App -->|风险| Risk[量子风险度量 §各节]
  App -->|模拟| QMC[量子蒙特卡洛 §各节]
  Opt --> Impl[§实现示例]
  Port --> Impl
  Risk --> Impl
  QMC --> Impl
