...
### 规约与模型在本领域的实例化 / Specification and Model Instantiation in Finance

在金融领域，算法规范与模型设计的实例化体现为：**业务规约**（如风险约束、收益目标、监管合规）→ **数学模型**（投资组合优化、期权定价、VaR 等）→ **算法与实现**（蒙特卡洛、梯度下降、交易执行）。规约-制品层次与 [项目哲科结构说明](../项目哲科结构说明.md)、[Stanford SEP Philosophy of Computer Science](https://plato.stanford.edu/entries/computer-science/) §2 对应。
...
- [12.5 金融算法应用 / Financial Algorithm Applications](#125-金融算法应用--financial-algorithm-applications)
  ...
  - [规约与模型在本领域的实例化 / Specification and Model Instantiation in Finance](#规约与模型在本领域的实例化--specification-and-model-instantiation-in-finance)
...
- [1. 形式化定义 / Formal Definitions](#1-形式化定义--formal-definitions)
  ...
  - [内容补充与思维表征 / Content Supplement and Thinking Representation](#内容补充与思维表征--content-supplement-and-thinking-representation)
    ...
    - [决策树：目标到算法选择 / Decision Tree](#决策树目标到算法选择--decision-tree)
...
```mermaid
mindmap
  root((金融算法<br/>Financial Algorithm))
    基本概念
      金融资产
        股票
        债券
...
**金融资产与投资组合（§1 形式化定义）的动机**：将资产收益、风险与投资组合权重形式化，便于讨论均值-方差优化、期权定价与 VaR 等正确性与约束。直观上与 09-03 优化理论（凸优化、约束）、10-22 量子算法在金融科技中的应用 衔接。

**与已有概念的联系**：投资组合优化与 09-03 二次规划、有效前沿一致；期权定价与 09-01 随机过程、二叉树对应；风险管理 VaR/CVaR 与 09-03 约束优化对应；与 12 应用领域 投资/交易/风控 为应用实践。

#### 概念属性表 / Concept Attribute Table

| 属性名 | 类型/范围 | 含义 | 备注 |
|--------|-----------|------|------|
| 金融资产 | 收益/波动 | 预期收益 $\mu$、协方差 $\Sigma$ | §1 |
| 投资组合 | 权重向量 $w$ | $\sum w_i=1$、约束 | §1、§2.1 |
| 均值-方差 | 目标函数 | $\min w^\top\Sigma w$ 或 $\max$ 夏普 | §2.1 |
| 期权定价 | 二叉树/Black-Scholes | 无套利、风险中性 | §2 |
| VaR/CVaR | 风险度量 | 尾部分位数/期望损失 | §4 |
| 量化交易 | 信号与执行 | 因子、回测、执行算法 | §3 |

#### 概念关系 / Concept Relations

| 源概念 | 目标概念 | 关系类型 | 说明 |
|--------|----------|----------|------|
| 金融算法应用 | 09-03 优化理论 | depends_on | 凸优化、约束 |
| 金融算法应用 | 10-22 量子金融 | depends_on | 量子优化/定价 |
| 核心算法(§2) | 量化交易(§3) | applies_to | 组合与定价支撑交易 |
| 风险管理(§4) | 核心算法(§2) | applies_to | VaR 依赖分布与组合 |
| 本文 | 12 应用领域 | applies_to | 投资/交易/风控 §5 |
...
**§1 投资组合形式化**与 **§2 核心算法**：均值-方差有效前沿由二次规划最优性保证；期权定价无套利由二叉树/BS 推导保证；与 09-03 凸优化与约束论证衔接。
...
| 概念/算法 | 风险维度 | 收益维度 | 适用场景 | 典型复杂度/备注 |
|-----------|----------|----------|----------|------------------|
| 现代投资组合(MPT) | 方差/协方差 | 预期收益 | 资产配置、有效前沿 | 二次规划 $O(n^3)$ §2.1 |
| 期权定价 | 波动率 | 无套利价格 | 衍生品估值 | 二叉树 $O(n)$ / BS 闭式 §2 |
| VaR/CVaR | 尾部分位数/期望损失 | — | 风险限额、监管 | 历史/蒙特卡洛 §4 |
| 量化交易 | 回测风险 | 因子收益 | 择时/选股/执行 | §3 |
...
flowchart TD
  Start([金融算法目标])
  Start --> Goal{主要目标?}
  Goal -->|投资组合| Core[核心算法 §2]
  Goal -->|期权/衍生品| Option[期权定价 §2]
  Goal -->|风险度量| Risk[风险管理 §4]
  Goal -->|交易执行| Quant[量化交易 §3]
  Core --> MPT[现代投资组合理论 §2.1]
  Option --> Tree[二叉树/BS]
  Risk --> VaR[VaR/CVaR §4]
  Quant --> Signal[信号与执行 §3]
```
...
calculateExpectedShortfall returns varThreshold =
    let tailReturns = filter (\leq varThreshold) returns
