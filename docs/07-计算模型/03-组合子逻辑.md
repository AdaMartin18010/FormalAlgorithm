...
组合子逻辑与计算模型可与 **MIT 18.404 Theory of Computation**、**CMU 15-251** 等课程对标。课程与模块映射见 [国际课程对标表](../国际课程对标表.md)。
...
- [3.1.4 内容补充与思维表征 / Content Supplement and Thinking Representation](#314-内容补充与思维表征--content-supplement-and-thinking-representation)
...
## 3.1.4 内容补充与思维表征 / Content Supplement and Thinking Representation

> 本节按 [内容补充与思维表征全面计划方案](../内容补充与思维表征全面计划方案.md) **只补充、不删除**。标准见 [内容补充标准](../内容补充标准-概念定义属性关系解释论证形式证明.md)、[思维表征模板集](../思维表征模板集.md)。

**解释与直观**：组合子逻辑用固定组合子（S、K、I 等）表示可计算函数，无需变量绑定；与 λ 演算等价（抽象算法），为无变量实现与类型组合子提供基础（见 02-λ演算、05-类型理论）。

**概念属性表**：S、K、I — SKI 组合子；B、C、K、W — BCKW；归约规则 — S f g x → f x (g x) 等；与 λ 演算 — 抽象算法将 λ 项译为组合子。

**概念关系**：组合子逻辑 —equivalent_to— λ 演算（见 02）；与 05-类型理论 — 类型组合子逻辑（见 §3.5）；与 01-图灵机 — Church-Turing 等价链（见 08）。

**概念依赖图**：SKI/BCKW → 组合子逻辑；抽象算法 → λ 演算等价；类型组合子 → 05-简单类型论；见 02、08。

**思维导图**：组合子逻辑 → SKI、BCKW、归约、正规形式、类型组合子 → λ 演算、类型论、图灵完备。

**多维矩阵**：SKI/BCKW — 组合子集；无类型/类型 — 与 05 对应；与 λ 演算 — 等价与翻译。

**公理定理推理证明决策树**：组合子与归约规则 → 图灵完备；与 λ 演算等价（抽象算法）；类型组合子 → 见 §3.5。

**应用决策建模树**：无变量实现 → 组合子解释器（见 §3.6）；类型论对应 → 见 05-类型系统；可计算性 → 见 01、02、08。
...
1. **Wikipedia - Combinatory Logic**: <https://en.wikipedia.org/wiki/Combinatory_logic>
   - 组合子逻辑的Wikipedia条目，包含SKI组合子和BCKW组合子（截至2025年1月11日）。

2. **Wikipedia - Lambda Calculus**: <https://en.wikipedia.org/wiki/Lambda_calculus>
   - λ演算的Wikipedia条目，介绍组合子逻辑与λ演算的关系（截至2025年1月11日）。
