---
title: 7.7 神经网络计算模型 / Neural Network Computational Models
version: 1.1
status: maintained
...
## 7.7 神经网络计算模型 / Neural Network Computational Models

### 摘要 / Executive Summary

- 统一神经网络的形式化定义、感知机、多层感知机与深度学习模型。
- 建立神经网络在机器学习与人工智能中的地位。

### 关键术语与符号 / Glossary

- 神经网络、人工神经元、感知机、多层感知机、卷积神经网络、递归神经网络、反向传播。
- 术语对齐与引用规范：`docs/术语与符号总表.md`，`01-基础理论/00-撰写规范与引用指南.md`

### 术语与符号规范 / Terminology & Notation

- 神经网络（Neural Network）：由互连的人工神经元组成的计算模型。
- 人工神经元（Artificial Neuron）：神经网络的基本计算单元。
- 反向传播（Backpropagation）：训练神经网络的学习算法。
- 记号约定：`w` 表示权重，`b` 表示偏置，`f` 表示激活函数。
...
神经网络计算模型可与 **CMU 10-606 Math for ML**、**MIT 6.046** 及机器学习/深度学习课程对标。课程与模块映射见 [国际课程对标表](../国际课程对标表.md)。
...
- [7.7 神经网络计算模型 / Neural Network Computational Models](#77-神经网络计算模型--neural-network-computational-models)
...
- [基本概念 / Basic Concepts](#基本概念--basic-concepts)
  - [人工神经元 / Artificial Neuron](#人工神经元--artificial-neuron)
  - [神经网络的组成要素 / Components of Neural Networks](#神经网络的组成要素--components-of-neural-networks)
  - [内容补充与思维表征 / Content Supplement and Thinking Representation](#内容补充与思维表征--content-supplement-and-thinking-representation)
- [基础神经网络实现 / Basic Neural Network Implementation](#基础神经网络实现--basic-neural-network-implementation)
  - [感知机模型 / Perceptron Model](#感知机模型--perceptron-model)
  - [多层感知机 / Multi-Layer Perceptron](#多层感知机--multi-layer-perceptron)
- [深度学习模型 / Deep Learning Models](#深度学习模型--deep-learning-models)
  - [卷积神经网络 / Convolutional Neural Networks](#卷积神经网络--convolutional-neural-networks)
- [递归神经网络 / Recurrent Neural Networks](#递归神经网络--recurrent-neural-networks)
  - [LSTM实现 / LSTM Implementation](#lstm实现--lstm-implementation)
- [学习算法理论 / Learning Algorithm Theory](#学习算法理论--learning-algorithm-theory)
...
- [*本文档提供了神经网络计算模型的完整形式化框架，包括感知机、多层感知机、反向传播、卷积神经网络、循环神经网络和深度学习的理论基础、形式化定义和实现示例。*](#本文档提供了神经网络计算模型的完整形式化框架包括感知机多层感知机反向传播卷积神经网络循环神经网络和深度学习的理论基础形式化定义和实现示例)
- [知识导航](#知识导航)
- [学习目标](#学习目标)
...
**解释与直观**：神经网络由人工神经元与连接权重构成，通过前向传播与反向传播进行函数逼近与模式识别；感知机、MLP、CNN、RNN 形成表达能力与适用场景的层次。

**概念属性表**：人工神经元 $y=f(\sum w_i x_i+b)$ — 权重、偏置、激活函数；网络 — 层、连接、学习算法；见定义 1.1 与组成要素。

**概念关系**：神经网络计算模型 —extends— 07-计算模型；与 09-算法理论 — 学习算法、优化；与 01-图灵机 — 表达力与可计算性（通用近似）。

**概念依赖图**：神经元 → 层 → 前馈/循环网络；反向传播 → 梯度与优化；CNN/RNN → 归纳偏置与适用任务。

**思维导图**：神经网络 → 感知机、MLP、CNN、RNN、学习算法 → 分类/回归/序列/表示学习。

**多维矩阵**：感知机/MLP/CNN/RNN — 结构、适用数据、归纳偏置；监督/无监督/强化 — 学习范式；激活函数 — 表达能力。

**论证与证明衔接**：定义 1.1 形式化神经元；通用近似定理（见学习算法理论）；收敛性与复杂度见 09-算法理论。

**决策树**：任务类型(图像/序列/表格) → CNN/RNN/MLP；数据量与标注 → 监督/无监督/强化；资源约束 → 深度与宽度选型。

**公理定理推理证明决策树**：神经元定义 → 层与网络；通用近似 → 存在性证明；反向传播 → 梯度计算正确性。

**应用决策建模树**：视觉/语言/控制 → 选 CNN/RNN/Transformer 等；训练流程 → 损失、优化器、正则；部署 → 剪枝、量化（见 09）。
...
1. [McCullochPitts1943] McCulloch, W. S., & Pitts, W. (1943). "A Logical Calculus of the Ideas Immanent in Nervous Activity". *The Bulletin of Mathematical Biophysics*, 5(4), 115-133. DOI: 10.1007/BF02478259
   - **McCulloch-Pitts神经元模型的开创性论文**，人工神经网络的奠基之作。本文档的基础概念参考此论文。
...
2. [Rosenblatt1958] Rosenblatt, F. (1958). "The Perceptron: A Probabilistic Model for Information Storage and Organization in the Brain". *Psychological Review*, 65(6), 386-408. DOI: 10.1037/h0042519
   - **Rosenblatt感知机的开创性论文**，第一个可学习的神经网络模型。本文档的感知机部分参考此论文。
...
3. [RumelhartHintonWilliams1986] Rumelhart, D. E., Hinton, G. E., & Williams, R. J. (1986). "Learning Representations by Back-Propagating Errors". *Nature*, 323(6088), 533-536. DOI: 10.1038/323533a0
   - **Rumelhart-Hinton-Williams反向传播算法的经典论文**，深度学习的基础。本文档的反向传播部分参考此论文。
...
4. [HochreiterSchmidhuber1997] Hochreiter, S., & Schmidhuber, J. (1997). "Long Short-Term Memory". *Neural Computation*, 9(8), 1735-1780. DOI: 10.1162/neco.1997.9.8.1735
   - Hochreiter-Schmidhuber LSTM的开创性论文，解决了RNN的长期依赖问题。本文档的LSTM部分参考此论文。
...
*本文档提供了神经网络计算模型的完整形式化框架，包括感知机、多层感知机、反向传播、卷积神经网络、循环神经网络和深度学习的理论基础、形式化定义和实现示例。*
---
