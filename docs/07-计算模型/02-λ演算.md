...
λ演算与计算模型可与 **MIT 18.404 Theory of Computation**、**CMU 15-251** 等课程对标。课程与模块映射见 [国际课程对标表](../国际课程对标表.md)。
...
**解释与直观**：λ演算用变量、抽象与应用刻画「函数与计算」；$\beta$-归约与 Church-Rosser 保证正规化；与图灵机、递归函数等价（Church-Turing），为 05-类型理论、03-形式化证明（Curry-Howard）提供语法基础。

**概念属性表**：λ项 — 变量、抽象 $\lambda x.M$、应用 $M N$；$\beta$-归约 — $(\lambda x.M)N \to M[N/x]$；Church-Rosser — 归约合流；正规形式 — 无 $\beta$-可约式。

**概念关系**：λ演算 —equivalent_to— 图灵机、递归函数（见 01、02、08）；与 05-简单类型论、02-依赖类型论 — 项与类型（Curry-Howard）；与 07-组合子逻辑 — 无变量表示（见 03）。

**概念依赖图**：λ项与归约 → 可计算性；Church-Rosser → 正规化；与图灵机/递归等价（见 01、02、08）；类型论（见 05）。

**思维导图**：λ演算 → 语法、归约、Church-Rosser、类型 λ → 类型论、证明论、程序设计语言。

**多维矩阵**：无类型/简单类型/多态/依赖 — 与 05 对应；$\beta$/$\eta$ — 归约；图灵完备 — 与 01、02 等价。

**公理定理推理证明决策树**：项与归约定义 → Church-Rosser；与图灵机等价（见 08）；类型 λ → 05-简单类型论。

**应用决策建模树**：程序设计语言语义 → λ演算与类型（见 05、08）；形式化证明 → Curry-Howard（见 03、05）；可计算性 → 见 02-递归理论、08-等价性。
...
- `07-计算模型/08-计算模型等价性理论.md` - 计算模型等价性理论（λ演算与图灵机的等价性）
- `07-计算模型/01-图灵机.md` - 图灵机（与λ演算的等价性）
- `05-类型理论/02-依赖类型论.md` - 依赖类型论（基于λ演算的类型系统）
- `09-算法理论/01-算法基础/22-算法六维分类框架.md` - 算法六维分类框架（计算模型维度）
- `view/算法全景梳理-2025-01-11.md` - 算法全景梳理（包含λ演算概述）
- `view/VIEW内容总索引-2025-01-11.md` - VIEW文件夹完整索引

### 知识体系位置 / Knowledge System Position

本文档属于 **07-计算模型** 模块，是计算模型理论的核心文档，为函数式编程和类型系统提供理论基础。

### VIEW文件夹相关文档 / VIEW Folder Related Documents

- `view/算法全景梳理-2025-01-11.md` §2.1 - 计算模型（λ演算）
