---
title: 6.3 直觉逻辑 / Intuitionistic Logic
version: 1.1
status: maintained
...
## 6.3 直觉逻辑 / Intuitionistic Logic

### 摘要 / Executive Summary

- 统一直觉逻辑的形式化定义、构造性证明与克里普克语义。
- 建立直觉逻辑与经典逻辑的对比框架。

### 关键术语与符号 / Glossary

- 直觉逻辑、构造性证明、排中律、克里普克语义、直觉主义。
- 术语对齐与引用规范：`docs/术语与符号总表.md`，`01-基础理论/00-撰写规范与引用指南.md`

### 术语与符号规范 / Terminology & Notation

- 直觉逻辑（Intuitionistic Logic）：拒绝排中律的构造性逻辑系统。
- 构造性证明（Constructive Proof）：必须提供构造的存在性证明。
- 克里普克语义（Kripke Semantics）：直觉逻辑的可能世界语义。
- 记号约定：`⊢` 表示推导，`⊨` 表示语义蕴含，`⊥` 表示矛盾。
...
- [6.3 直觉逻辑 / Intuitionistic Logic](#63-直觉逻辑--intuitionistic-logic)
...
- [1. 基本概念 (Basic Concepts)](#1-基本概念-basic-concepts)
  - [1.1 直觉逻辑定义 (Definition of Intuitionistic Logic)](#11-直觉逻辑定义-definition-of-intuitionistic-logic)
  - [1.2 直觉逻辑与经典逻辑的区别 (Difference from Classical Logic)](#12-直觉逻辑与经典逻辑的区别-difference-from-classical-logic)
  - [1.3 直觉逻辑的基本原理 (Basic Principles of Intuitionistic Logic)](#13-直觉逻辑的基本原理-basic-principles-of-intuitionistic-logic)
- [1.4 内容补充与思维表征 / Content Supplement and Thinking Representation](#14-内容补充与思维表征--content-supplement-and-thinking-representation)
- [2. 直觉主义 (Intuitionism)](#2-直觉主义-intuitionism)
  - [2.1 直觉主义哲学 (Intuitionistic Philosophy)](#21-直觉主义哲学-intuitionistic-philosophy)
  - [2.2 直觉主义数学 (Intuitionistic Mathematics)](#22-直觉主义数学-intuitionistic-mathematics)
  - [2.3 直觉主义与形式主义 (Intuitionism vs Formalism)](#23-直觉主义与形式主义-intuitionism-vs-formalism)
- [3. 构造性证明 (Constructive Proofs)](#3-构造性证明-constructive-proofs)
  - [3.1 构造性证明定义 (Definition of Constructive Proofs)](#31-构造性证明定义-definition-of-constructive-proofs)
  - [3.2 构造性证明技术 (Constructive Proof Techniques)](#32-构造性证明技术-constructive-proof-techniques)
  - [3.3 非构造性证明的拒绝 (Rejection of Non-Constructive Proofs)](#33-非构造性证明的拒绝-rejection-of-non-constructive-proofs)
- [4. 克里普克语义 (Kripke Semantics)](#4-克里普克语义-kripke-semantics)
  - [4.1 克里普克模型 (Kripke Models)](#41-克里普克模型-kripke-models)
  - [4.2 克里普克语义 (Kripke Semantics)](#42-克里普克语义-kripke-semantics)
  - [4.3 克里普克语义的性质 (Properties of Kripke Semantics)](#43-克里普克语义的性质-properties-of-kripke-semantics)
- [5. 实现示例 (Implementation Examples)](#5-实现示例-implementation-examples)
  - [5.1 直觉逻辑解析器 (Intuitionistic Logic Parser)](#51-直觉逻辑解析器-intuitionistic-logic-parser)
  - [5.2 克里普克模型实现 (Kripke Model Implementation)](#52-克里普克模型实现-kripke-model-implementation)
  - [5.3 构造性证明系统 (Constructive Proof System)](#53-构造性证明系统-constructive-proof-system)
  - [5.4 直觉逻辑测试 (Intuitionistic Logic Testing)](#54-直觉逻辑测试-intuitionistic-logic-testing)
...
### 1.1 直觉逻辑定义 (Definition of Intuitionistic Logic)
...
### 1.2 直觉逻辑与经典逻辑的区别 (Difference from Classical Logic)
...
### 1.3 直觉逻辑的基本原理 (Basic Principles of Intuitionistic Logic)
...
**解释与直观**：直觉逻辑拒绝排中律 $P \vee \neg P$，证明即构造；与构造性证明（见 03-形式化证明/03-构造性证明）、依赖类型论与 Curry-Howard 对应，为 Coq/Agda 等证明助手的逻辑基础。

**概念属性表**：直觉逻辑 — 无排中律、双重否定消去；BHK 解释 — 证明即构造；与经典逻辑 — 经典逻辑为直觉逻辑+排中律；Kripke 语义 — 可能世界语义。

**概念关系**：直觉逻辑 —depends_on— 命题/一阶逻辑（见 01、02）；与 03-形式化证明/构造性证明 — 证明即构造；与 05-依赖类型论 — Curry-Howard 对应。

**概念依赖图**：命题逻辑 → 去掉排中律 → 直觉逻辑；BHK → 证明即程序；Curry-Howard → 05-依赖类型、08-Coq/Agda。

**思维导图**：直觉逻辑 → BHK、Kripke 语义、与经典对比、构造性证明 → 类型论、证明助手。

**多维矩阵**：经典/直觉 — 排中律；自然演绎/序列演算 — 证明系统（见 03-证明系统）；命题/一阶 — 表达能力。

**公理定理推理证明决策树**：直觉逻辑公理与规则 → 无排中律；BHK 解释 → 证明即构造；与类型论对应 → Curry-Howard（见 05）。

**应用决策建模树**：形式化于 Coq/Agda → 见 05、08；构造性证明 → 见 03-形式化证明/03-构造性证明；反证法限制 → 见 03-形式化证明/04-反证法。

---

## 2. 直觉主义 (Intuitionism)

### 2.1 直觉主义哲学 (Intuitionistic Philosophy)
...
### 2.2 直觉主义数学 (Intuitionistic Mathematics)
...
### 2.3 直觉主义与形式主义 (Intuitionism vs Formalism)
...
## 3. 构造性证明 (Constructive Proofs)

### 3.1 构造性证明定义 (Definition of Constructive Proofs)
...
### 3.2 构造性证明技术 (Constructive Proof Techniques)
...
### 3.3 非构造性证明的拒绝 (Rejection of Non-Constructive Proofs)
...
## 4. 克里普克语义 (Kripke Semantics)

### 4.1 克里普克模型 (Kripke Models)
...
### 4.3 克里普克语义的性质 (Properties of Kripke Semantics)
...
**情况4：** 假设 $\phi = \psi_1 \rightarrow \psi_2$。
...
### 5.1 直觉逻辑解析器 (Intuitionistic Logic Parser)
...
### 5.2 克里普克模型实现 (Kripke Model Implementation)
...
### 5.3 构造性证明系统 (Constructive Proof System)
...
### 5.4 直觉逻辑测试 (Intuitionistic Logic Testing)
