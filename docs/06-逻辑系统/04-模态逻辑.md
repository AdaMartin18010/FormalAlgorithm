...
**解释与直观**：模态逻辑用 $\Box$（必然）与 $\Diamond$（可能）刻画命题的必然性与可能性；可能世界语义（Kripke 模型）为 $\Box/\Diamond$ 提供语义，用于知识、信念、时序与程序验证（见 07-时序逻辑、08-形式化验证）。

**概念属性表**：$\Box \phi$ — 必然；$\Diamond \phi$ — 可能（$\Diamond \phi \equiv \neg \Box \neg \phi$）；Kripke 模型 $\mathcal{M}=(W,R,V)$ — 世界、可达关系、赋值；公理 K、T、4、5 — 对应关系性质。

**概念关系**：模态逻辑 —extends— 命题逻辑（见 01）；与 07-时序逻辑、09-时序逻辑理论 — 时序与程序逻辑；与 08-形式化验证 — 规范与模型检测。

**概念依赖图**：命题逻辑 → $\Box/\Diamond$ → 模态逻辑；Kripke 模型 → 可能世界语义；公理 K/T/4/5 → 对应关系自反/传递/欧性等；见 07、08。

**思维导图**：模态逻辑 → $\Box/\Diamond$、Kripke 语义、公理与对应、多模态 → 知识逻辑、时序逻辑、程序验证。

**多维矩阵**：K/T/S4/S5 — 公理与框架；可能世界/邻域 — 语义；知识/信念/时序 — 应用（见 07、08）。

**公理定理推理证明决策树**：公理 K、T、4、5 → 对应关系性质；Kripke 完备性；与经典嵌入；见 §2–§3。

**应用决策建模树**：程序/规范验证 → 时序逻辑与模型检测（见 07、08）；知识与信念 → 多主体逻辑；形式化验证 → 见 08-形式化验证、10-形式化验证的高级技术。
