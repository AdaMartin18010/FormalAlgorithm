---
title: 6.2 一阶逻辑 / First-Order Logic
version: 1.1
status: maintained
...
## 6.2 一阶逻辑 / First-Order Logic

### 摘要 / Executive Summary

- 统一一阶逻辑的形式化定义、语法、语义与证明系统。
- 建立一阶逻辑的完备性理论与模型论基础。

### 关键术语与符号 / Glossary

- 一阶逻辑、量词、谓词、项、公式、结构、赋值、满足关系、完备性定理。
- 术语对齐与引用规范：`docs/术语与符号总表.md`，`01-基础理论/00-撰写规范与引用指南.md`

### 术语与符号规范 / Terminology & Notation

- 一阶逻辑（First-Order Logic）：包含量词的形式逻辑系统。
- 全称量词（Universal Quantifier）：`∀` 表示"对所有"。
- 存在量词（Existential Quantifier）：`∃` 表示"存在"。
- 记号约定：`⊢` 表示语法推导，`⊨` 表示语义蕴含，`≡` 表示逻辑等价。
...
- [6.2 一阶逻辑 / First-Order Logic](#62-一阶逻辑--first-order-logic)
...
- [1. 基本概念 (Basic Concepts)](#1-基本概念-basic-concepts)
  - [1.1 一阶逻辑定义 (Definition of First-Order Logic)](#11-一阶逻辑定义-definition-of-first-order-logic)
  - [1.2 一阶逻辑与命题逻辑的区别 (Difference from Propositional Logic)](#12-一阶逻辑与命题逻辑的区别-difference-from-propositional-logic)
  - [1.3 一阶逻辑的基本要素 (Basic Elements of First-Order Logic)](#13-一阶逻辑的基本要素-basic-elements-of-first-order-logic)
...
- [2. 语法 (Syntax)](#2-语法-syntax)
  - [2.1 语言定义 (Language Definition)](#21-语言定义-language-definition)
  - [2.2 项的定义 (Definition of Terms)](#22-项的定义-definition-of-terms)
  - [2.3 公式的定义 (Definition of Formulas)](#23-公式的定义-definition-of-formulas)
  - [2.4 自由变量和约束变量 (Free and Bound Variables)](#24-自由变量和约束变量-free-and-bound-variables)
- [3. 语义 (Semantics)](#3-语义-semantics)
  - [3.1 结构定义 (Definition of Structure)](#31-结构定义-definition-of-structure)
  - [3.2 赋值 (Assignment)](#32-赋值-assignment)
  - [3.3 满足关系 (Satisfaction Relation)](#33-满足关系-satisfaction-relation)
- [4. 证明系统 (Proof Systems)](#4-证明系统-proof-systems)
  - [4.1 自然演绎系统 (Natural Deduction System)](#41-自然演绎系统-natural-deduction-system)
  - [4.2 希尔伯特系统 (Hilbert System)](#42-希尔伯特系统-hilbert-system)
  - [4.3 序列演算 (Sequent Calculus)](#43-序列演算-sequent-calculus)
- [5. 完备性 (Completeness)](#5-完备性-completeness)
  - [5.1 完备性定理 (Completeness Theorem)](#51-完备性定理-completeness-theorem)
  - [5.2 紧致性定理 (Compactness Theorem)](#52-紧致性定理-compactness-theorem)
  - [5.3 勒文海姆-斯科伦定理 (Löwenheim-Skolem Theorem)](#53-勒文海姆-斯科伦定理-löwenheim-skolem-theorem)
- [6. 实现示例 (Implementation Examples)](#6-实现示例-implementation-examples)
  - [6.1 一阶逻辑解析器 (First-Order Logic Parser)](#61-一阶逻辑解析器-first-order-logic-parser)
  - [6.2 一阶逻辑解释器 (First-Order Logic Interpreter)](#62-一阶逻辑解释器-first-order-logic-interpreter)
  - [6.3 一阶逻辑证明系统 (First-Order Logic Proof System)](#63-一阶逻辑证明系统-first-order-logic-proof-system)
  - [6.4 一阶逻辑测试 (First-Order Logic Testing)](#64-一阶逻辑测试-first-order-logic-testing)
...
### 1.1 一阶逻辑定义 (Definition of First-Order Logic)
...
### 1.2 一阶逻辑与命题逻辑的区别 (Difference from Propositional Logic)
...
### 1.3 一阶逻辑的基本要素 (Basic Elements of First-Order Logic)
...
**解释与直观**：一阶逻辑在命题逻辑上加入量词 $\forall/\exists$、谓词与函数符号，可形式化数学与程序规范；可靠性与完备性（Gödel）连接语法与语义，为形式化验证与类型论提供逻辑基础。

**概念属性表**：一阶语言 $\mathcal{L}$ — 常量、函数、谓词符号；公式 — 原子、联结词、量词；$\Gamma \vdash \phi$ / $\Gamma \models \phi$ — 可证与语义蕴涵；紧致性 — 无穷可满足性。

**概念关系**：一阶逻辑 —extends— 命题逻辑（见 01）；与 03-形式化证明 — 公理与推理规则；与 05-依赖类型论 — 命题即类型在一阶片段对应；与 08-形式化验证 — 规范与证明。

**概念依赖图**：一阶语言 → 项与公式；公理与规则 → 可证性；可靠性/完备性 → $\vdash$ 与 $\models$；见 03、05、08。

**思维导图**：一阶逻辑 → 语法、语义、量词、可靠性与完备性、紧致性 → 形式化数学、验证、类型论。

**多维矩阵**：命题/一阶/高阶 — 表达能力；经典/直觉 — 语义；自然演绎/序列演算 — 证明系统（见 03）。

**公理定理推理证明决策树**：公理与规则 → 可证性；Gödel 完备性定理；紧致性定理；Löwenheim-Skolem；见 §2–§3。

**应用决策建模树**：形式化数学/规范 → 一阶语言与公理（见 03、08）；自动证明/SMT → 一阶片段与可判定子类；类型论对应 → 见 05-依赖类型系统与数理逻辑。

---

## 2. 语法 (Syntax)

### 2.1 语言定义 (Language Definition)
...
### 2.3 公式的定义 (Definition of Formulas)
...
### 2.4 自由变量和约束变量 (Free and Bound Variables)
...
### 3.1 结构定义 (Definition of Structure)
...
### 3.3 满足关系 (Satisfaction Relation)
...
## 4. 证明系统 (Proof Systems)

### 4.1 自然演绎系统 (Natural Deduction System)
...
### 5.1 完备性定理 (Completeness Theorem)
...
### 5.3 勒文海姆-斯科伦定理 (Löwenheim-Skolem Theorem)
...
### 6.1 一阶逻辑解析器 (First-Order Logic Parser)
...
### 6.2 一阶逻辑解释器 (First-Order Logic Interpreter)
...
### 6.3 一阶逻辑证明系统 (First-Order Logic Proof System)
...
### 6.4 一阶逻辑测试 (First-Order Logic Testing)
...
1. **Wikipedia - First-Order Logic**: <https://en.wikipedia.org/wiki/First-order_logic>
   - 一阶逻辑的Wikipedia条目，包含语法、语义和完备性定理（截至2025年1月11日）。

2. **Wikipedia - Model Theory**: <https://en.wikipedia.org/wiki/Model_theory>
   - 模型论的Wikipedia条目，详细介绍一阶逻辑的语义理论（截至2025年1月11日）。

3. **Wikipedia - Gödel's Completeness Theorem**: <https://en.wikipedia.org/wiki/G%C3%B6del%27s_completeness_theorem>
   - 哥德尔完备性定理的Wikipedia条目，一阶逻辑的核心结果（截至2025年1月11日）。
