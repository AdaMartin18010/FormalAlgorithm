...
- [1. 形式化定义 / Formal Definitions](#1-形式化定义--formal-definitions)
  ...
  - [1.3 凸包](#13-凸包)
  - [1.4 点在多边形内](#14-点在多边形内)
- [2. 核心思路与算法框架](#2-核心思路与算法框架)
  - [2.1 一维重叠判定](#21-一维重叠判定)
  - [2.2 点在多边形内判定](#22-点在多边形内判定)
  - [2.3 Andrew 凸包算法](#23-andrew-凸包算法)
- [3. 经典题目详解](#3-经典题目详解)
  - [3.1 LeetCode 223 — 矩形面积](#31-leetcode-223--矩形面积)
  ...
  - [3.2 LeetCode 587 — 安装栅栏](#32-leetcode-587--安装栅栏)
...
- [4. 复杂度分析体系](#4-复杂度分析体系)
  - [4.1 计算几何基础操作复杂度](#41-计算几何基础操作复杂度)
  - [4.2 凸包算法复杂度对比](#42-凸包算法复杂度对比)
- [5. 正确性证明框架](#5-正确性证明框架)
  - [5.1 叉积方向判定公式的证明](#51-叉积方向判定公式的证明)
  - [5.2 凸包算法的不变式证明](#52-凸包算法的不变式证明)
- [6. 思维表征](#6-思维表征)
  - [6.1 概念依赖图](#61-概念依赖图)
  - [6.2 算法选择决策树](#62-算法选择决策树)
  - [6.3 概念依赖与证明路径图](#63-概念依赖与证明路径图)
  - [6.4 多维矩阵概念对比](#64-多维矩阵概念对比)
- [7. 常见错误与反模式](#7-常见错误与反模式)
  - [7.1 叉积坐标顺序错误](#71-叉积坐标顺序错误)
  - [7.2 Andrew 算法弹栈条件错误](#72-andrew-算法弹栈条件错误)
  - [7.3 凸包端点重复](#73-凸包端点重复)
  - [7.4 矩形面积中的整数溢出](#74-矩形面积中的整数溢出)
  - [7.5 点在多边形内的射线法边界情况](#75-点在多边形内的射线法边界情况)
  - [7.6 凸包算法的共线点重复](#76-凸包算法的共线点重复)
- [8. 自测问题](#8-自测问题)
  - [问题 1：叉积与点积的区别](#问题-1叉积与点积的区别)
  - [问题 2：一维重叠判定的充分必要性](#问题-2一维重叠判定的充分必要性)
  - [问题 3：Andrew 算法为何需要排序](#问题-3andrew-算法为何需要排序)
  - [问题 4：凸包上的共线点处理](#问题-4凸包上的共线点处理)
  - [问题 5：叉积在面积计算中的应用](#问题-5叉积在面积计算中的应用)
- [9. 学习目标](#9-学习目标)
- [10. 知识导航](#10-知识导航)
- [参考文献](#参考文献)
...
- `09-算法理论/04-高级算法理论/计算几何.md` — 计算几何的理论框架与高级算法
...
**定义 1.9** (简单多边形 / Simple Polygon)
由 $n$ 个顶点 $p_0, p_1, \dots, p_{n-1}$ 按顺序连接而成的闭合链，且边与边之间除相邻边共享顶点外不相交。

**定义 1.10** (点在多边形内 / Point in Polygon)
给定简单多边形 $P$ 和点 $q$，$q$ 在 $P$ **内部**当且仅当从 $q$ 发出的任意射线与 $P$ 的边界相交奇数次；$q$ 在 $P$ **外部**当且仅当相交偶数次。

> **注意**: 上述定义要求射线不经过任何顶点。实际算法中通过“上闭下开”约定处理顶点情况。
...
**问题**: 给定简单多边形 $P$（顶点按顺序给出）和点 $q$，判定 $q$ 是否在 $P$ 内部。
...
PointInPolygon(P, q):
    count ← 0
    for each edge (pi, pj) of P:
        if ray from q to +∞ intersects (pi, pj):
            count ← count + 1
    return count is odd
```

**核心思想**: 从点 $q$ 向右发出水平射线，统计该射线与多边形边界的交点数。若交点数为奇数，则点在多边形内部；若为偶数，则在外部。

**边界处理**:

- 若射线恰好穿过顶点，采用“上闭下开”约定：只统计该顶点为边的高端点的情况，避免重复计数。
- 若点恰好在边界上，根据题目要求返回 true 或 false。
...
```mermaid
graph LR
    Def1[定义: 向量与点] --> Def2[定义: 叉积]
    Def2 --> Thm1[定理: 方向判定公式]
    Thm1 --> Alg1[Andrew 凸包算法]
    Def2 --> App1[应用: LC 587]
    Thm1 --> App2[应用: 矩形重叠 LC 223]
    Alg1 --> Proof[循环不变式证明]
```
...
**错误**: 使用射线法判定一点是否在多边形内时，未处理射线恰好穿过顶点或与边重合的情况。

**正确做法**: 采用“上闭下开”约定：当射线穿过顶点时，只统计该顶点为边的上端点的情况。这样可以避免顶点被重复计数。若点恰好在边界上，根据题目要求返回 true 或 false。

### 7.6 凸包算法的共线点重复

**错误**: 在 Andrew 算法中，上下凸壳的端点（最左点和最右点）在合并时被重复加入结果集。

**正确做法**: 合并时去掉下凸壳的最后一个点和上凸壳的最后一个点（或根据实现方式去掉各自的首尾重复点），避免结果中出现重复坐标。
