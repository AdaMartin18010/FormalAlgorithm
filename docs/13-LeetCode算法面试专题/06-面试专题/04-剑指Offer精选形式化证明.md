...
**定理 4.3**：递归函数 `printNumbers(digit, n)` 恰好生成所有 n 位数字排列一次。
...
**定理 4.4**：`reversePrint(head)` 返回以 `head` 为头的链表的逆序值数组。
...
**定理 4.6**：`buildTree(preorder, inorder)` 正确重建二叉树。
...
**定理 4.7**：`isMatch(a, b)` 返回 true 当且仅当以 `a` 为根的子树包含 `b` 为子结构。
...
**定理 4.9**：到达 $(i,j)$ 的最优路径必然经过 $(i-1,j)$ 或 $(i,j-1)$ 的最优路径。
