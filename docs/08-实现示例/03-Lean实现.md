...
> 说明：本项目包含 Lean4 形式化证明工程（`examples/lean_proofs/`），配置 `lakefile.toml`，涵盖排序正确性、图论性质、数论归纳等定理证明。所有 `.lean` 文件遵循 Lean4 语法规范。
...
- **依赖类型论**: `05-类型理论/02-依赖类型论.md` §2.1-§2.3
- **证明系统**: `03-形式化证明/01-证明系统.md` §2-§4

#### 相关理论 (Related)

- **同伦类型论**: `05-类型理论/03-同伦类型论.md` §3.1-§3.5
- **依赖类型系统与数理逻辑**: `05-类型理论/05-依赖类型系统与数理逻辑.md` §5.6.6（最新版本特性实践）、§5.8.3（Lean 4.8.0新特性）

#### 反向链接 (Backward Links)

本文档被以下文档引用：

- `05-类型理论/05-依赖类型系统与数理逻辑.md` §交叉引用导航、§5.6.5-§5.6.6

### 适用范围与局限 / Scope and Limitations

本项目中的 Lean 内容以**概念演示与教学**为主，非工业级验证工程。

- **工业适用性**：证明助手的工业采纳受证明维护成本、工具链与自动化程度制约；**工业适用性与工具选型需另行调研**。
- **项目定位**：本仓库为教育资源，侧重规范与模型理解；详见 [03-形式化证明/01-证明系统](../03-形式化证明/01-证明系统.md) §适用范围与局限 与 [09-算法理论/04-高级算法理论](../09-算法理论/04-高级算法理论/) 中验证理论文档。
...
Lean 实现将基本概念与类型系统、定理证明、数学库、实现示例结合。与 05-依赖类型论、03-形式化证明、08-04 形式化验证衔接；§3.1–§3.5 形成完整表征。

#### 概念属性表 / Concept Attribute Table

| 属性名 | 类型/范围 | 含义 | 备注 |
|--------|-----------|------|------|
| 基本概念(Lean 定义、历史、应用领域) | 基本概念 | §3.1 | 与 05、03、08-04 对照 |
| 类型系统、定理证明、数学库、实现示例 | 模块/示例 | 表达力、可验证性、适用场景 | §3.2–§3.5 |
| 类型系统/定理证明/数学库 | 对比 | §3.2–§3.4 | 多维矩阵 |

#### 概念关系 / Concept Relations

| 源概念 | 目标概念 | 关系类型 | 说明 |
|--------|----------|----------|------|
| Lean 实现 | 05、03、08-04 | depends_on | 依赖类型论、形式化证明、形式化验证 |
| Lean 实现 | 08-01/02/05/06 实现示例 | relates_to | 实现实践 |
...
定理证明正确性见 §3.3；与 03 形式化证明论证衔接；类型系统正确性见 §3.2。
...
|-----------|--------|----------|----------|------|
| 类型系统/定理证明/数学库 | §3.2–§3.4 | §3.2–§3.4 | §3.2–§3.4 | — |
...
flowchart TD
  Start([目标])
  Start --> Goal{目标?}
  Goal -->|类型/证明/数学库| Mod[类型系统或定理证明或数学库 §3.2–§3.4]
  Mod --> Impl[§3.5 实现示例]
```
...
flowchart TD
  Need([应用需求])
  Need --> App{需求?}
  App -->|形式化/证明/验证| Meth[Lean 实现模块或它语言 §3.5 实现示例]
  Meth --> Impl[§3.5]
```
