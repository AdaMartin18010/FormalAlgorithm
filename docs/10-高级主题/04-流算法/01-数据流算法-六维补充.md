...
### 8.1 Count-Min Sketch 的数学保证（完整形式化）

**定理 8.1.1** (Count-Min Sketch 误差界 [Cormode and Muthukrishnan 2005])
设流 $\sigma$ 的频率向量为 $f$，Count-Min Sketch 使用宽度 $w = \lceil e/\epsilon \rceil$、深度 $d = \lceil \ln(1/\delta) \rceil$，并采用两两独立哈希函数。对任意元素 $x$，估计值 $\hat{f}_x$ 满足
$$f_x \le \hat{f}_x \le f_x + \epsilon \|f\|_1$$
以至少 $1-\delta$ 的概率成立。

*证明*：对固定深度 $j$，计数器 $C[j][h_j(x)]$ 的期望为
$$\mathbb{E}[C[j][h_j(x)]] = f_x + \frac{\|f\|_1 - f_x}{w} \le f_x + \frac{\epsilon \|f\|_1}{e}.$$
由马尔可夫不等式，
$$\Pr\left[C[j][h_j(x)] - f_x \ge \epsilon \|f\|_1\right] \le \frac{\mathbb{E}[C[j][h_j(x)] - f_x]}{\epsilon \|f\|_1} \le \frac{1}{e}.$$
取 $d$ 个深度的最小值，所有 $j$ 均超界的概率至多为 $e^{-d} \le \delta$。又 $C[j][h_j(x)] \ge f_x$ 恒成立，故下界显然。∎

---

### 8.2 HyperLogLog 的数学保证

**定理 8.2.1** (HyperLogLog 相对误差界 [Flajolet et al. 2007])
设流中不同元素的真实基数为 $n$，使用 $m = 2^b$ 个寄存器的 HyperLogLog 估计值为 $\hat{n}$。当 $n \to \infty$ 时，
$$\frac{\hat{n} - n}{n} \xrightarrow{d} \mathcal{N}\left(0, \frac{1.04^2}{m}\right).$$
即标准相对误差约为 $1.04/\sqrt{m}$。

*证明概要*：

1. 对每个寄存器 $j$，定义随机变量 $R_j = \max_{x \in S_j} \rho(h_2(x))$，其中 $S_j$ 为落入该桶的元素集合。
2. 当 $n$ 足够大时，每个桶的期望基数为 $n/m$。对均匀哈希，$R_j$ 近似服从 $\Pr[R_j \le k] \approx \exp(-n/(m 2^{k+1}))$。
3. 原始 LogLog 估计器使用算术平均，受离群值影响大；HyperLogLog 改用**调和平均** $\hat{n} = \alpha_m m^2 / \sum_{j=1}^m 2^{-R_j}$，显著降低了方差。
4. 通过分析指示变量的协方差并应用中心极限定理，可得 $\hat{n}$ 的渐近正态分布，其渐近标准差为 $1.04/\sqrt{m}$。∎
...
2024–2025 年，流算法在超图（hypergraph）处理上取得重要进展。[Hypergraph_Sparsification_2024] 提出了近最优大小的线性草图（linear sketches）用于超图割稀疏化，空间复杂度达到了理论下界的多项式倍数。[Hypergraph_Sparsification_2025] 进一步将结果推广到仅插入（insertion-only）与有界删除（bounded-deletion）流模型，为动态超图分析提供了高效工具。

### 9.2 最大有向割的流算法

最大有向割（Maximum Directed Cut）是图分割中的核心问题。[Streaming_Directed_Cut_2025] 通过局部算法（local algorithms）设计了新型的流算法，在单次扫描模型下获得了优于传统贪心策略的近似比，展示了局部计算与全局流处理结合的新范式。

### 9.3 学习增强的流算法

机器学习预测被成功引入流算法设计。[Learning_Augmented_Correlation_Clustering_2025] 在学习增强的流模型中，为相关聚类（correlation clustering）问题提供了更紧的近似比界限。这类方法利用预测信息减少 sketch 的更新次数或优化抽样策略，代表了流算法与实际数据特征深度融合的方向。

---

## 参考文献

[1] Vitter, J. S. (1985). "Random sampling with a reservoir". *ACM Transactions on Mathematical Software*, 11(1), 37–57. DOI: 10.1145/3147.3165

[2] Cormode, G., and Muthukrishnan, S. (2005). "An improved data stream summary: the Count-Min Sketch and its applications". *Journal of Algorithms*, 55(1), 58–75. DOI: 10.1016/j.jalgor.2003.12.001

[3] Flajolet, P., Fusy, É., Gandouet, O., and Meunier, F. (2007). "HyperLogLog: the analysis of a near-optimal cardinality estimation algorithm". *Discrete Mathematics and Theoretical Computer Science*, AH, 127–146.

[4] Streaming_Directed_Cut_2025. (2025). "Streaming Algorithms via Local Algorithms for Maximum Directed Cut". In *Proceedings of the 36th Annual ACM-SIAM Symposium on Discrete Algorithms (SODA 2025)*.

[5] Hypergraph_Sparsification_2024. (2024). "Near-Optimal Size Linear Sketches for Hypergraph Cut Sparsifiers". In *Proceedings of the 65th IEEE Annual Symposium on Foundations of Computer Science (FOCS 2024)*.

[6] Hypergraph_Sparsification_2025. (2025). "Near-Optimal Hypergraph Sparsification in Insertion-Only and Bounded-Deletion Streams". In *Proceedings of the 42nd International Colloquium on Automata, Languages, and Programming (ICALP 2025)*.
