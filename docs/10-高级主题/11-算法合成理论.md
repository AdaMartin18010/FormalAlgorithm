---
title: 10.11 算法合成理论 / Algorithm Synthesis Theory
version: 1.0
status: maintained
...
flowchart TD
  ...
  Form -->|逻辑| Ded[演绎 §合成策略]
  Form -->|示例| Ind[归纳 §合成策略]
  Form -->|组合| Comb[组合 §合成策略]
  Form -->|转换| Trans[转换 §合成策略]
  Ded --> Meth[§合成方法]
  Ind --> Meth
  Comb --> Meth
  Trans --> Meth
...
flowchart TD
  Need([应用需求])
  Need --> App{需求类型?}
  App -->|规约驱动开发| Spec[对应合成策略与方法 §应用领域]
  App -->|程序修复| Fix[对应合成策略与方法 §应用领域]
  App -->|自动实现| Auto[对应合成策略与方法 §应用领域]
  Spec --> Impl[§实现示例]
  Fix --> Impl
  Auto --> Impl
```
