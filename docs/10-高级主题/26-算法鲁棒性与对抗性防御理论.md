...
算法鲁棒性与对抗性防御理论将鲁棒性定义与对抗攻击、防御机制结合。与 10-25 可解释性与透明度、10-29 可信AI治理衔接；§基本概念、§对抗性攻击技术、§防御机制、§鲁棒性评估形成完整表征。

#### 概念属性表 / Concept Attribute Table

| 属性名 | 类型/范围 | 含义 | 备注 |
|--------|-----------|------|------|
| 算法鲁棒性、对抗性攻击 | 基本概念 | §基本概念 | 与 10-25、10-29 对照 |
| 对抗性攻击技术、防御机制、鲁棒性评估 | 技术/评估 | 威胁模型、防御强度、适用场景 | §对抗性攻击技术、§防御机制、§鲁棒性评估 |
| 白盒/黑盒、对抗训练/输入预处理/运行时防御 | 对比 | §各节 | 多维矩阵 |

#### 概念关系 / Concept Relations

| 源概念 | 目标概念 | 关系类型 | 说明 |
|--------|----------|----------|------|
| 算法鲁棒性与对抗性防御理论 | 10-25、10-29 | depends_on | 可解释性与治理基础 |
| 算法鲁棒性与对抗性防御理论 | 12 应用领域 | applies_to | 安全实践 |
...
鲁棒性度量见 §鲁棒性评估；对抗训练收敛性见 §防御机制；与 10-25 论证衔接。

#### 思维导图：本章概念结构 / Mind Map

```mermaid
graph TD
  Robust[算法鲁棒性与对抗性防御理论]
  Robust --> BC[基本概念]
  Robust --> Attack[攻击技术]
  Robust --> Def[防御机制]
  Robust --> Eval[评估]
  Attack --> White[白盒/黑盒]
  Def --> Train[对抗训练/输入预处理/运行时]
```

#### 多维矩阵：攻击与防御对比 / Multi-Dimensional Comparison

| 概念/技术 | 威胁模型 | 防御强度 | 适用场景 | 备注 |
|-----------|----------|----------|----------|------|
| 白盒/黑盒攻击、对抗训练/输入预处理/运行时防御 | §各节 | §各节 | §各节 | — |

#### 决策树：威胁模型到防御选择 / Decision Tree

```mermaid
flowchart TD
  Start([威胁模型])
  Start --> Type{类型?}
  Type -->|白盒/黑盒| Def[对抗训练或输入预处理或运行时防御 §各节]
  Def --> Impl[§实现示例]
```
...
flowchart TD
  Need([应用需求])
  Need --> App{需求类型?}
  App -->|部署环境/威胁等级/成本约束| Meth[防御机制与鲁棒性评估方法 §实现示例]
  Meth --> Impl[§实现示例]
```
