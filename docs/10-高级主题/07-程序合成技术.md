---
title: 10.7 程序合成技术 / Program Synthesis Techniques
version: 1.0
status: maintained
...
flowchart TD
  Start([程序合成需求])
  Start --> Spec{规约形式?}
  Spec -->|语法约束| Syn[语法引导 各节]
  Spec -->|逻辑约束| Con[约束引导 各节]
  Spec -->|示例/数据| ML[ML引导 各节]
  Spec -->|示例归纳| Ind[归纳 各节]
  Spec -->|公理/逻辑| Ded[演绎 各节]
  Syn --> Impl[实现]
  Con --> Impl
  ML --> Impl
```
...
flowchart TD
  Need([应用需求：程序合成])
  Need --> App{应用场景?}
  App -->|规约驱动开发| Syn[语法/约束引导 各节]
  App -->|程序修复| Con[约束引导 各节]
  App -->|自动实现| ML[ML/归纳/演绎 各节]
  Syn --> Impl[各节 实现]
  Con --> Impl
  ML --> Impl
```
