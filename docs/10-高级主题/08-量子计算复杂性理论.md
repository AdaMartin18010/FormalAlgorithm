---
title: 10.8 量子计算复杂性理论 / Quantum Computational Complexity Theory
version: 1.0
status: maintained
...
flowchart TD
  Start([量子复杂度需求])
  Start --> Type{问题类型?}
  Type -->|判定| Class[量子复杂性类 §量子复杂性类]
  Type -->|优化| Class
  Start --> Model{模型选型?}
  Model -->|图灵机/电路| Def[§基本概念 量子计算模型]
  Model -->|绝热/测量基| Def
  Class --> BQP[BQP §量子复杂性类]
  Class --> QMA[QMA §量子复杂性类]
```
...
flowchart TD
  Need([应用需求：量子计算复杂性])
  Need --> App{应用场景?}
  App -->|量子算法复杂度分析| Def[模型与 T_A/S_A 见本文]
  App -->|下界证明| LB[下界与归约 见本文]
  App -->|复杂度类归类| Class[量子复杂性类 §量子复杂性类]
  Def --> Impl[见本文]
  LB --> Impl
  Class --> Impl
```
