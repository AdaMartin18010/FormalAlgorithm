...
- $N = \{1, 2, \cdots, n\}$: 玩家集合
- $A_i$: 玩家 $i$ 的行动集
- $u_i: A_1 \times A_2 \times \cdots \times A_n \rightarrow \mathbb{R}$: 玩家 $i$ 的收益函数

**策略组合**: $a = (a_1, a_2, \cdots, a_n) \in A = A_1 \times \cdots \times A_n$
...
策略组合 $a^* = (a_1^*, \cdots, a_n^*)$ 是**纯策略纳什均衡**，当且仅当：
$$\forall i \in N, \forall a_i \in A_i: u_i(a_i^*, a_{-i}^*) \geq u_i(a_i, a_{-i}^*)$$

其中 $a_{-i}^* = (a_1^*, \cdots, a_{i-1}^*, a_{i+1}^*, \cdots, a_n^*)$。
...
2. **整体最佳回应**: $BR(\sigma) = BR_1(\sigma_{-1}) \times \cdots \times BR_n(\sigma_{-n})$
