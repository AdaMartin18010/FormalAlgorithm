---
title: 10.18 量子算法复杂度理论 / Quantum Algorithm Complexity Theory
version: 1.0
status: maintained
...
flowchart TD
  Start([资源类型])
  Start --> Type{类型?}
  Type -->|查询| Query[量子查询复杂度 §各节]
  Type -->|通信| Comm[量子通信复杂度 §各节]
  Type -->|空间| Space[量子空间复杂度 §各节]
  Type -->|时间| Time[量子时间 §各节]
  Query --> Bound[下界 §量子下界理论]
  Comm --> Bound
  Space --> Bound
  Time --> Bound
```
...
flowchart TD
  Need([应用需求])
  Need --> App{需求类型?}
  App -->|量子算法复杂度分析| Anal[选查询/通信/空间/时间复杂度 见本文]
  App -->|下界证明| Bound[选对应复杂度与下界 见本文]
  Anal --> Impl[本文]
  Bound --> Impl
```
