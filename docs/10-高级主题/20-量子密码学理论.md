...
- [概述 / Overview](#概述--overview)
  - [内容补充与思维表征 / Content Supplement and Thinking Representation](#内容补充与思维表征--content-supplement-and-thinking-representation)
    ...
    - [多维矩阵：量子密码协议对比 / Multi-Dimensional Comparison](#多维矩阵量子密码协议对比--multi-dimensional-comparison)
    - [决策树：需求到协议选择 / Decision Tree](#决策树需求到协议选择--decision-tree)
...
量子密码学理论利用量子力学原理提供无条件安全协议；量子密钥分发、量子数字签名、量子随机数、量子安全哈希与后量子密码学构成理论栈。与 07-量子计算模型、10-04 量子信息论、10-16 量子算法安全衔接；§概述、§量子密钥分发及后续各节形成完整表征。

#### 概念属性表 / Concept Attribute Table

| 属性名 | 类型/范围 | 含义 | 备注 |
|--------|-----------|------|------|
| 量子密码学理论 | 理论框架 | 无条件安全 | §概述 |
| 量子密钥分发/数字签名/随机数/哈希/后量子 | 协议 | 安全假设、复杂度、适用场景 | §量子密钥分发及后续各节 |
| 与 10-16 对照 | — | — | 多维矩阵对照 |
...
| 源概念 | 目标概念 | 关系类型 | 说明 |
|--------|----------|----------|------|
| 量子密码学理论 | 07-量子计算模型、10-04 量子信息论、10-16 量子算法安全 | depends_on | 计算与安全基础 |
| 量子密码学理论 | 09-01-15 量子算法、04-复杂度 | relates_to | Shor 等、密码安全假设 |
...
无条件安全形式化见 §概述；各协议正确性与安全性见 §各节；与 10-04、10-16 论证衔接。
...
#### 多维矩阵：量子密码协议对比 / Multi-Dimensional Comparison

| 概念/协议 | 安全假设 | 复杂度 | 适用场景 | 备注 |
|-----------|----------|--------|----------|------|
| 量子密钥分发/数字签名/随机数/哈希/后量子 | §各节 | §各节 | §各节 | 与 10-16 对照 |

#### 决策树：需求到协议选择 / Decision Tree

```mermaid
flowchart TD
  Start([需求])
  Start --> Need{需求?}
  Need -->|密钥交换/签名/随机数/哈希/后量子| Proto[对应量子密码协议 §各节]
  Proto --> Impl[本文]
```
...
flowchart TD
  Need([应用需求])
  Need --> App{需求类型?}
  App -->|安全通信/身份认证/随机数/抗量子| Proto[对应量子密码协议 见本文]
  Proto --> Impl[本文]
```
