...
flowchart TD
  Start([应用领域])
  Start --> Dom{领域?}
  Dom -->|函数式| FP[函子/单子 §1.3]
  Dom -->|类型论| TT[范畴语义 §1.3]
  Dom -->|并发| Conc[进程代数 §1.3]
  Dom -->|数据库| DB[数据模型 §1.3]
  FP --> Impl[§1.4 实现示例]
  TT --> Impl
  Conc --> Impl
...
flowchart TD
  Need([应用需求])
  Need --> App{需求类型?}
  App -->|类型语义| TT[范畴/函子 §1.3]
  App -->|单子效应| Mon[单子 §1.2.4]
  App -->|并发| Conc[进程代数 §1.3]
  App -->|数据库| DB[数据模型 §1.3]
  TT --> Impl[§1.4 实现]
  Mon --> Impl
  Conc --> Impl
...
testMonadLaws ma f g =
  let law1 = (return a >\geq f) == f a
      law2 = (ma >\geq return) == ma
      law3 = ((ma >\geq f) >\geq g) == (ma >\geq (\x -> f x >\geq g))
  in law1 && law2 && law3
