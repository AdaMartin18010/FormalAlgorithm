...
flowchart TD
  Start([目标])
  Start --> Goal{目标?}
  Goal -->|训练/推理/能效| Meth[脉冲训练或事件驱动或硬件优化 §各节]
  Meth --> Impl[§应用案例]
```
...
flowchart TD
  Need([应用需求])
  Need --> App{需求类型?}
  App -->|边缘AI/实时控制| Meth[脉冲神经网络或事件驱动算法 §应用案例]
  Meth --> Impl[§应用案例]
```
...
- 硬件与边缘系统：`10-高级主题/30-边缘计算中的算法系统.md`，`10-高级主题/30-边缘计算中的算法系统.md`
