...
> 📚 **概念引用**：关于 **同伦类型论** 的完整定义，详见 [`docs/知识图谱/concept_nodes.yaml`](../知识图谱/concept_nodes.yaml) 中 `homotopy_type_theory` 的定义，或参见 [`docs/05-类型理论/03-同伦类型论.md`](../05-类型理论/03-同伦类型论.md)。

> 同伦类型论的基础概念（定义、历史、应用领域）已在主文档中完整阐述。本节直接进入高级应用。
