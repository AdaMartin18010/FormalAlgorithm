...
集合 $A$ 是有限的当且仅当存在 $n \in \mathbb{N}$ 使得 $|A| = |\{0, 1, \cdots, n-1\}|$。
...
| **更大基数** | $2^{\mathfrak{c}}, 2^{2^{\mathfrak{c}}}, \cdots$ | $\mathcal{P}(\mathbb{R}), \mathcal{P}(\mathcal{P}(\mathbb{R}))$ |
...
构造新数 $x = 0.x_1 x_2 x_3 x_4 \cdots$，其中 $x_i \neq d_{ii}$。
