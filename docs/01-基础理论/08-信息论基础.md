...
- [概述 / Overview](#概述--overview)
  ...
  - [信息·数据·数据结构的定义 / Definition of Information, Data, and Data Structures](#信息数据数据结构的定义--definition-of-information-data-and-data-structures)
...
- [*本文档展示了信息论的基础理论，通过严格的数学定义和算法实现，为信息处理提供了理论基础。*](#本文档展示了信息论的基础理论通过严格的数学定义和算法实现为信息处理提供了理论基础)
- [知识导航](#知识导航)
- [学习目标](#学习目标)
...
### 信息·数据·数据结构的定义 / Definition of Information, Data, and Data Structures

**定义 1.4** (信息·数据·数据结构的形式化定义) 设：
**Definition 1.4** (Formal Definition of Information, Data, and Data Structures) Let:

| 符号 | 含义 | 形式化描述 |
|------|------|-----------|
| **Σ** | 有限字符集（字母表） | 如 Σ = {0,1} 或 ASCII |
| **数据 D** | Σ* 中的有限串，记作 `D ∈ Σ*` | 任何可存储、可传输的离散对象 |
| **信息量 I(D)** | 对数据 D 的不确定性度量 | **Shannon 熵** `H(D) = -∑_x p(x) log₂ p(x)`（随机源）<br>**Kolmogorov 复杂度** `K(D)`（最短自描述长度） |
| **数据结构 DS** | 具有 **底层表示** `repr: DS → Σ*` 与 **操作集合** `Ops(DS) = {op₁,…,op_k}` 的抽象代数对象 | 例：二叉搜索树 `BST = (Node, left, right, key, …)`，`Ops = {search, insert, delete}` |

| Symbol | Meaning | Formal Description |
|--------|---------|-------------------|
| **Σ** | Finite alphabet | e.g., Σ = {0,1} or ASCII |
| **Data D** | Finite string in Σ*, denoted as `D ∈ Σ*` | Any storable, transmittable discrete object |
| **Information Content I(D)** | Uncertainty measure of data D | **Shannon entropy** `H(D) = -∑_x p(x) log₂ p(x)` (random source)<br>**Kolmogorov complexity** `K(D)` (shortest self-description length) |
| **Data Structure DS** | Abstract algebraic object with **underlying representation** `repr: DS → Σ*` and **operation set** `Ops(DS) = {op₁,…,op_k}` | Example: Binary search tree `BST = (Node, left, right, key, …)`, `Ops = {search, insert, delete}` |

> **信息-数据-结构的关系 / Information-Data-Structure Relationship**:
>
> - `I(D)` 为 **数据的内在难度**（压缩极限）。
> - `I(D)` is the **intrinsic difficulty of data** (compression limit).
>
> - **数据结构** 通过 **组织方式** 把 `D` 的 **查询/更新** 代价映射到 **时间/空间复杂度**。
> - **Data structures** map the **query/update** cost of `D` to **time/space complexity** through **organization**.
>
> - **算法** 在特定 **计算模型** 上实现对 `DS` 的操作，从而 **利用或克服信息的不可压缩性**。
...
- `01-基础理论/07-概率与统计基础.md` - 概率与统计基础（信息论的数学基础）
- `04-算法复杂度/06-信息论下界.md` - 信息论下界（信息论在算法复杂度中的应用）
- `04-算法复杂度/05-通信复杂度.md` - 通信复杂度（信息论在通信复杂度中的应用）
- 相关内容已整合到本文档（原 `view/算法全景梳理-2025-01-11.md` §1.2, §3.8）

### 知识体系位置 / Knowledge System Position

本文档属于 **01-基础理论** 模块，是信息论的基础文档，为算法复杂度分析和通信复杂度分析提供理论基础。

### VIEW文件夹相关文档 / VIEW Folder Related Documents

- 相关内容已整合到本文档和 `04-算法复杂度/06-信息论下界.md`（原 `view/算法全景梳理-2025-01-11.md` §1.2, §3.8）
