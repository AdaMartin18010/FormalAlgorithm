...
**解释与直观**：递归可枚举集是可被部分递归函数枚举的集合，等价于「图灵机半判定」的集合；递归集严格含于递归可枚举集，且 $A$ 递归当且仅当 $A$ 与 $\overline{A}$ 皆递归可枚举。

**概念属性表**

| 属性名 | 类型/范围 | 含义 |
|--------|-----------|------|
| $\mathcal{RE}$ | 集合类 | 递归可枚举集 |
| $\mathcal{R}$ | 集合类 | 递归集 |
| 枚举函数 | 部分递归 $f$ | $A = \mathrm{range}(f)$ |
| 半判定 | 图灵机 | 接受恰为 $A$ 中元素 |

**概念关系**：递归可枚举 —equivalent_to— 图灵机半判定、部分递归函数值域；$\mathcal{R} \subset \mathcal{RE}$；$A \in \mathcal{R} \Leftrightarrow A \in \mathcal{RE} \wedge \overline{A} \in \mathcal{RE}$。

**概念依赖图**：一般递归/图灵机 → 定义域/半判定 → 递归可枚举；递归 = 递归可枚举且补集递归可枚举。

**思维导图**：递归可枚举 → 枚举定义、半判定、投影定理 → 递归、不可解性（见 05）。

**多维矩阵**：递归 / 递归可枚举 — 判定与半判定；枚举与图灵机接受域等价；$K$ 递归可枚举但非递归。

**公理定理推理证明决策树**：定义 2.1.1（枚举）→ $\mathcal{RE}$；定理 $\mathcal{R} \subset \mathcal{RE}$；定理 $A \in \mathcal{R} \Leftrightarrow A,\overline{A} \in \mathcal{RE}$。

**应用决策建模树**：证集合递归可枚举 → 给枚举算法或图灵机半判定；证非递归 → 归约至 $K$ 或停机（见 05）。
...
1. **Wikipedia - Recursively Enumerable Set**: <https://en.wikipedia.org/wiki/Recursively_enumerable_set>
   - 递归可枚举集的Wikipedia条目，包含基本定义、性质和与图灵机的关系（截至2025年1月11日）。

2. **Wikipedia - Computability Theory**: <https://en.wikipedia.org/wiki/Computability_theory>
   - 可计算性理论的Wikipedia条目，介绍递归可枚举性在可计算性理论中的核心地位（截至2025年1月11日）。

3. **Wikipedia - Turing Machine**: <https://en.wikipedia.org/wiki/Turing_machine>
   - 图灵机的Wikipedia条目，说明递归可枚举集与图灵可识别语言的关系（截至2025年1月11日）。
