---
title: 2.3 一般递归函数 / General Recursive Functions
version: 1.1
status: maintained
...
- 建立一般递归函数的形式化定义、μ-递归与部分递归函数。
- 建立一般递归函数在递归理论中的核心地位。

### 关键术语与符号 / Glossary

- 一般递归函数、μ-递归函数、部分递归函数、递归可枚举性、递归不可解性。
- 术语对齐与引用规范：`docs/术语与符号总表.md`，`01-基础理论/00-撰写规范与引用指南.md`

### 术语与符号规范 / Terminology & Notation

- 一般递归函数（General Recursive Function）：包含所有可计算函数的函数类。
- μ-递归函数（μ-Recursive Function）：使用μ算子构造的递归函数。
- 部分递归函数（Partial Recursive Function）：可能在某些输入上未定义的递归函数。
- 记号约定：`GR` 表示一般递归函数类，`μ` 表示μ算子，`f` 表示函数。

### 交叉引用导航 / Cross-References

- 递归函数定义：参见 `02-递归理论/01-递归函数定义.md`。
- 原始递归函数：参见 `02-递归理论/02-原始递归函数.md`。
...
**解释与直观**：一般递归函数由基本函数经合成、原始递归与 μ 算子得到；与图灵可计算、λ 可定义等价（Church-Turing 论题），部分函数对应「可能不终止」的计算。

**概念属性表**

| 属性名 | 类型/范围 | 含义 |
|--------|-----------|------|
| μ-算子 | 最小化 | $\mu y[f(\vec{x},y)=0]$，无界搜索 |
| $\mathcal{GR}$ | 函数类 | 一般递归（全函数部分） |
| $\mathcal{PC}$ | 函数类 | 部分可计算函数 |
| 图灵可计算 | 等价类 | $\mathcal{GR} = \mathcal{TR}$ |

**概念关系**：一般递归 —equivalent_to— 图灵可计算、λ 可定义；一般递归 —superset— 原始递归；μ-递归 —equivalent_to— 一般递归。

**概念依赖图**：原始递归 + μ → 一般递归；一般递归 ↔ 图灵机（见 07-计算模型）。

**思维导图**：一般递归 → μ-递归、图灵可计算、递归可枚举 → 停机问题、递归不可解（见 04、05）。

**多维矩阵**：原始递归 / 一般递归 / 图灵可计算 — 包含与等价；μ-算子 — 引入部分性；递归可枚举 — 一般递归定义域。

**公理定理推理证明决策树**：定义 μ-算子 → μ-递归函数类；Church-Turing → $\mathcal{GR}=\mathcal{TR}$；递归可枚举集 = 部分递归函数定义域。

**应用决策建模树**：证可计算性（含部分函数）→ 给一般递归或图灵机；证递归可枚举 → 给枚举函数（见 04-递归可枚举性）。
