...
**解释与直观**：原始递归函数由基本函数经合成与原始递归模式封闭得到；所有原始递归函数全函数且可计算，但阿克曼函数表明存在可计算全函数非原始递归。

**概念属性表**

| 属性名 | 类型/范围 | 含义 |
|--------|-----------|------|
| 基本函数 | 零/后继/投影 | $\mathcal{PR}$ 的生成元 |
| 合成 | 运算 | $f(g_1,\ldots,g_k)$ |
| 原始递归 | 模式 | $h(\vec{x},0)=f(\vec{x}),\, h(\vec{x},y+1)=g(\vec{x},y,h(\vec{x},y))$ |
| $\mathcal{PR}$ | 函数类 | 最小包含基本函数且对合成与原始递归封闭的类 |

**概念关系**：原始递归函数 —subset— 一般递归；原始递归 —depends_on— 基本函数与函数构造（见 01-递归函数定义）。

**概念依赖图**：基本函数 → 合成/原始递归 → $\mathcal{PR}$；$\mathcal{PR}$ → 一般递归（见 03-一般递归函数）。

**思维导图**：原始递归 → 基本函数、合成、原始递归模式 → 算术运算、有界量化、阿克曼非原始递归。

**多维矩阵**：基本函数 / 合成 / 原始递归 — 封闭性；$\mathcal{PR}$ — 全函数、可计算、非全体可计算函数（阿克曼）。

**公理定理推理证明决策树**：定义 2.1.1（$\mathcal{PR}$）→ 归纳于构造；定理：阿克曼函数可计算但非原始递归。

**应用决策建模树**：证函数原始递归 → 用基本函数+合成+原始递归构造；需无界搜索 → 用一般递归/μ-递归（见 03）。
