...
渐进分析与大 O/Ω/Θ 可与 **MIT 6.006/6.046**、**CMU 15-451**、**Stanford CS 161**、**Berkeley CS 170** 等课程对标。课程与模块映射见 [国际课程对标表](../国际课程对标表.md)。

### 可知边界与哲科参考 / Knowable Boundaries and Philosophy

渐进下界与**可知边界**的认识论刻画见 [04-复杂度类](04-复杂度类.md) §可知边界与认识论参考、[Stanford SEP - Philosophy of Computer Science](https://plato.stanford.edu/entries/computer-science/)、[项目哲科结构说明](../项目哲科结构说明.md)。
...
**解释与直观**：渐进分析用 $O/\Omega/\Theta/o/\omega$ 刻画函数渐近关系，忽略常数与低阶项；主定理给出分治递推 $T(n)=aT(n/b)+f(n)$ 的紧界，为算法复杂度提供统一工具。

**概念属性表**：$O(g)$ — 渐近上界；$\Omega(g)$ — 渐近下界；$\Theta(g)$ — 渐近紧界；$o(g)/\omega(g)$ — 严格渐近；主定理 — $a,b$ 与 $f$ 比较得 $T(n)$。

**概念关系**：渐进分析 —applies_to— 01-时间复杂度、02-空间复杂度；主定理 —depends_on— 递推与正则条件；与 04-复杂度类 — 用渐进记号定义类。

**概念依赖图**：渐进记号定义 → $O/\Omega/\Theta$ 性质；主定理 → 分治复杂度；递归树/主定理 — 两种方法（见 §2–§3）。

**思维导图**：渐进分析 → 渐进记号、主定理、递归树、代入法 → 时间/空间复杂度、复杂度类。

**多维矩阵**：$O/\Omega/\Theta/o/\omega$ — 含义与比较；主定理情形 1/2/3 — 条件与结论；递归树 vs 主定理 — 适用场景。

**公理定理推理证明决策树**：定义 $O/\Omega/\Theta$ → 传递性、自反性；主定理 — 情形划分与证明；递归树展开 → 与主定理一致。

**应用决策建模树**：求递推式紧界 → 主定理或递归树（见 §2–§3）；证 $f=O(g)$ → 找 $c,n_0$；算法比较 → 用 $\Theta$（见 01、02）。
...
> 💻 **代码示例引用**：排序算法与动态规划的实现示例及复杂度分析可参见 [`docs/示例库/01-标准示例集.md`](../示例库/01-标准示例集.md)。
...
- `view/算法全景梳理-2025-01-11.md` - 算法全景梳理（包含复杂度分析概述）
...
- `view/算法全景梳理-2025-01-11.md` §4 - 信息·数据·数据结构（复杂度分析）
- `view/算法全景梳理-2025-01-11.md` §5 - 信息通信中的算法复杂度（渐进分析）
- `view/VIEW内容总索引-2025-01-11.md` §4 - 内容主题索引（算法复杂度）
- `view/VIEW内容总索引-2025-01-11.md` - VIEW文件夹完整索引
