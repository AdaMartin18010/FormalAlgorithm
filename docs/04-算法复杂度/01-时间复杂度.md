...
渐进分析、复杂度类与下界可与以下国际课程对标：**MIT 18.404 Theory of Computation**、**MIT 6.006/6.046**、**CMU 15-451 Algorithm Design and Analysis**、**Stanford CS 161**、**Berkeley CS 170**。课程与模块映射及建议阅读顺序见 [国际课程对标表](../国际课程对标表.md)。

### 哲科结构参考 / Philosophy of Computer Science Reference

**下界与「可知」边界**：复杂度下界刻画了算法可达到的性能极限，属于认识论范畴。详见 [Stanford SEP - Philosophy of Computer Science](https://plato.stanford.edu/entries/computer-science/) §8 与 [项目哲科结构说明](../项目哲科结构说明.md)。
...
**解释与直观**：时间复杂度将算法「快慢」形式化为输入规模 $n$ 的函数 $T(n)$；渐进记号 $O/\Omega/\Theta$ 刻画上界/下界/紧界，为算法比较与下界证明提供统一语言。

**概念属性表**：$T(n)$ — 基本操作次数或步数；$O(g)$ — 渐近上界；$\Omega(g)$ — 渐近下界；$\Theta(g)$ — 渐近紧界；时间可构造 — 用于层次定理。

**概念关系**：时间复杂度 —depends_on— 计算模型与成本模型（§1）；与空间复杂度 —对比— 见 02-空间复杂度；渐进记号 —applies_to— 03-渐进分析、04-复杂度类。

**概念依赖图**：计算模型 → $T(n)$ 定义；渐进记号 → $O/\Omega/\Theta$；时间可构造 → 层次定理（见 04-复杂度类）。

**思维导图**：时间复杂度 → 最坏/平均/最好、渐进记号、主定理、时间可构造 → 复杂度类、下界。

**多维矩阵**：最坏/平均/最好 — 分析类型；$O/\Omega/\Theta/o/\omega$ — 渐进关系；RAM/图灵机 — 模型（见 07）。

**公理定理推理证明决策树**：定义 $T(n)$ → 渐进记号定义；时间可构造 → 层次定理；主定理 → 递归复杂度（见 03）。

**应用决策建模树**：比较算法 → 用 $O/\Theta$（见 §2）；证下界 → 用 $\Omega$ 或信息论（见 06）；形式化 → 见 09-算法理论。
...
1. **Wikipedia - Time Complexity**: <https://en.wikipedia.org/wiki/Time_complexity>
   - 时间复杂度的Wikipedia条目，包含基本定义和分析方法（截至2025年1月11日）。
...
5. ECCC (Electronic Colloquium on Computational Complexity): <https://eccc.weizmann.ac.il/>
   - 计算复杂性领域的预印本服务器。
...
- `04-算法复杂度/02-空间复杂度.md` - 空间复杂度（复杂度分析的另一个维度）
- `04-算法复杂度/03-渐进分析.md` - 渐进分析（时间复杂度分析的工具）
- `04-算法复杂度/04-复杂度类.md` - 复杂度类（时间复杂度分类）
- `04-算法复杂度/06-信息论下界.md` - 信息论下界（时间复杂度的下界理论）
- `09-算法理论/01-算法基础/01-算法设计理论.md` - 算法设计理论（时间复杂度在算法设计中的应用）
- `view/算法全景梳理-2025-01-11.md` - 算法全景梳理（包含复杂度分析概述）
- `view/VIEW内容总索引-2025-01-11.md` - VIEW文件夹完整索引

### 知识体系位置 / Knowledge System Position

本文档属于 **04-算法复杂度** 模块，是算法复杂度分析的基础文档，为算法设计和分析提供时间复杂度理论基础。

### VIEW文件夹相关文档 / VIEW Folder Related Documents

- `view/算法全景梳理-2025-01-11.md` §4 - 信息·数据·数据结构（复杂度分析）
- `view/算法全景梳理-2025-01-11.md` §5 - 信息通信中的算法复杂度（时间复杂度）
