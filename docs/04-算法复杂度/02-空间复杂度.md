---
title: 4.2 空间复杂度 / Space Complexity
version: 1.1
status: maintained
...
- 统一空间复杂度的形式化定义、渐进分析与空间复杂度类关系。
- 建立时间-空间权衡的理论框架。

### 关键术语与符号 / Glossary

- 空间复杂度、辅助空间、工作空间、空间复杂度类、空间可构造性。
- 术语对齐与引用规范：`docs/术语与符号总表.md`，`01-基础理论/00-撰写规范与引用指南.md`

### 术语与符号规范 / Terminology & Notation

- 空间复杂度（Space Complexity）：算法执行过程中所需的内存空间大小。
- 辅助空间（Auxiliary Space）：除输入外算法需要的额外空间。
- 空间复杂度类（Space Complexity Classes）：L、NL、PSPACE、EXPSPACE等。
- 记号约定：`S(n)` 表示空间复杂度，`O`、`Ω`、`Θ` 表示渐进记号。
...
空间复杂度与时间-空间权衡可与 **MIT 6.006/6.046**、**CMU 15-451**、**Stanford CS 161**、**Berkeley CS 170** 等课程对标。课程与模块映射见 [国际课程对标表](../国际课程对标表.md)。

### 可知边界与哲科参考 / Knowable Boundaries and Philosophy

复杂度下界与**可知边界**的认识论刻画见 [04-复杂度类](04-复杂度类.md) §可知边界与认识论参考、[Stanford SEP - Philosophy of Computer Science](https://plato.stanford.edu/entries/computer-science/)、[项目哲科结构说明](../项目哲科结构说明.md)。
...
- [1. 基本概念 (Basic Concepts)](#1-基本概念-basic-concepts)
  - [1.1 空间复杂度定义 (Definition of Space Complexity)](#11-空间复杂度定义-definition-of-space-complexity)
  - [1.2 空间复杂度类型 (Types of Space Complexity)](#12-空间复杂度类型-types-of-space-complexity)
  - [1.3 空间复杂度记号 (Space Complexity Notation)](#13-空间复杂度记号-space-complexity-notation)
...
- [2. 空间复杂度分析 (Space Complexity Analysis)](#2-空间复杂度分析-space-complexity-analysis)
  - [2.1 递归空间复杂度 (Recursive Space Complexity)](#21-递归空间复杂度-recursive-space-complexity)
  - [2.2 动态规划空间复杂度 (Dynamic Programming Space Complexity)](#22-动态规划空间复杂度-dynamic-programming-space-complexity)
  - [2.3 图算法空间复杂度 (Graph Algorithm Space Complexity)](#23-图算法空间复杂度-graph-algorithm-space-complexity)
- [3. 空间复杂度类 (Space Complexity Classes)](#3-空间复杂度类-space-complexity-classes)
  - [3.1 对数空间类 (Logarithmic Space Classes)](#31-对数空间类-logarithmic-space-classes)
  - [3.2 多项式空间类 (Polynomial Space Classes)](#32-多项式空间类-polynomial-space-classes)
  - [3.3 指数空间类 (Exponential Space Classes)](#33-指数空间类-exponential-space-classes)
  - [3.4 空间复杂度类层次 (Space Complexity Class Hierarchy)](#34-空间复杂度类层次-space-complexity-class-hierarchy)
- [4. 空间构造 (Space Constructions)](#4-空间构造-space-constructions)
  - [4.1 空间可构造性 (Space Constructibility)](#41-空间可构造性-space-constructibility)
  - [4.2 空间归约 (Space Reductions)](#42-空间归约-space-reductions)
  - [4.3 空间完全性 (Space Completeness)](#43-空间完全性-space-completeness)
- [5. 空间下界理论 (Space Lower Bounds)](#5-空间下界理论-space-lower-bounds)
  ...
  - [5.3 时间-空间权衡 (Time-Space Trade-offs)](#53-时间-空间权衡-time-space-trade-offs)
- [6. 空间复杂度与时间复杂度 (Space vs Time Complexity)](#6-空间复杂度与时间复杂度-space-vs-time-complexity)
  - [6.1 时间-空间关系 (Time-Space Relationship)](#61-时间-空间关系-time-space-relationship)
  - [6.2 空间-时间权衡 (Space-Time Trade-offs)](#62-空间-时间权衡-space-time-trade-offs)
  - [6.3 并行空间复杂度 (Parallel Space Complexity)](#63-并行空间复杂度-parallel-space-complexity)
- [7. 实现示例 (Implementation Examples)](#7-实现示例-implementation-examples)
  - [7.1 空间复杂度分析工具 (Space Complexity Analysis Tool)](#71-空间复杂度分析工具-space-complexity-analysis-tool)
  - [7.2 空间优化示例 (Space Optimization Examples)](#72-空间优化示例-space-optimization-examples)
  - [7.3 图算法空间复杂度示例 (Graph Algorithm Space Complexity Examples)](#73-图算法空间复杂度示例-graph-algorithm-space-complexity-examples)
  - [7.4 空间复杂度测试 (Space Complexity Testing)](#74-空间复杂度测试-space-complexity-testing)
...
### 1.1 空间复杂度定义 (Definition of Space Complexity)
...
### 1.2 空间复杂度类型 (Types of Space Complexity)
...
### 1.3 空间复杂度记号 (Space Complexity Notation)
...
**解释与直观**：空间复杂度将算法「内存占用」形式化为 $S(n)$；与时间复杂度权衡（时间—空间折衷），辅助器相关分析（见 09-算法优化理论）。

**概念属性表**：$S(n)$ — 工作空间（不含输入）；$O/\Omega/\Theta$ — 空间渐进记号；递归深度 $d(n)$ — 与栈空间相关；辅助空间 — 除输入外的额外空间。

**概念关系**：空间复杂度 —depends_on— 计算模型（见 01-时间复杂度）；与时间复杂度 —对比— 时间—空间折衷；$PSPACE$ — 见 04-复杂度类。

**概念依赖图**：计算模型 → $S(n)$；递归深度 → 栈空间；$PSPACE$、$L$、$NL$ — 空间复杂度类（见 04）。

**思维导图**：空间复杂度 → 工作空间、递归空间、辅助空间、渐进记号 → 复杂度类、缓存优化。

**多维矩阵**：工作/辅助/总空间 — 空间类型；$O/\Theta$ — 渐进；$L/NL/PSPACE$ — 空间类（见 04）。

**公理定理推理证明决策树**：定义 $S(n)$ → 空间渐进记号；Savitch 定理 $NL \subseteq L^2$；空间层次定理。

**应用决策建模树**：分析内存占用 → 用 $S(n)$ 与递归深度；证空间下界 → 用 $\Omega$；选数据结构 → 见 09-算法理论/数据结构。

---

## 2. 空间复杂度分析 (Space Complexity Analysis)

### 2.1 递归空间复杂度 (Recursive Space Complexity)
...
### 2.2 动态规划空间复杂度 (Dynamic Programming Space Complexity)
...
### 2.3 图算法空间复杂度 (Graph Algorithm Space Complexity)
...
## 3. 空间复杂度类 (Space Complexity Classes)

### 3.1 对数空间类 (Logarithmic Space Classes)
...
### 3.2 多项式空间类 (Polynomial Space Classes)
...
### 3.3 指数空间类 (Exponential Space Classes)
...
### 3.4 空间复杂度类层次 (Space Complexity Class Hierarchy)
...
## 4. 空间构造 (Space Constructions)

### 4.1 空间可构造性 (Space Constructibility)
...
### 5.3 时间-空间权衡 (Time-Space Trade-offs)
...
## 6. 空间复杂度与时间复杂度 (Space vs Time Complexity)

### 6.1 时间-空间关系 (Time-Space Relationship)
...
### 6.2 空间-时间权衡 (Space-Time Trade-offs)
...
### 6.3 并行空间复杂度 (Parallel Space Complexity)
...
### 7.1 空间复杂度分析工具 (Space Complexity Analysis Tool)
...
### 7.2 空间优化示例 (Space Optimization Examples)
...
### 7.3 图算法空间复杂度示例 (Graph Algorithm Space Complexity Examples)
...
### 7.4 空间复杂度测试 (Space Complexity Testing)
...
- `04-算法复杂度/01-时间复杂度.md` - 时间复杂度（复杂度分析的另一个维度）
- `04-算法复杂度/03-渐进分析.md` - 渐进分析（空间复杂度分析的工具）
- `04-算法复杂度/04-复杂度类.md` - 复杂度类（空间复杂度分类）
- `09-算法理论/01-算法基础/02-数据结构理论.md` - 数据结构理论（空间复杂度在数据结构中的应用）
- `view/算法全景梳理-2025-01-11.md` - 算法全景梳理（包含复杂度分析概述）
- `view/VIEW内容总索引-2025-01-11.md` - VIEW文件夹完整索引

### 知识体系位置 / Knowledge System Position

本文档属于 **04-算法复杂度** 模块，是算法复杂度分析的基础文档，为算法设计和数据结构分析提供空间复杂度理论基础。

### VIEW文件夹相关文档 / VIEW Folder Related Documents

- `view/算法全景梳理-2025-01-11.md` §4 - 信息·数据·数据结构（复杂度分析）
- `view/算法全景梳理-2025-01-11.md` §5 - 信息通信中的算法复杂度（空间复杂度）
