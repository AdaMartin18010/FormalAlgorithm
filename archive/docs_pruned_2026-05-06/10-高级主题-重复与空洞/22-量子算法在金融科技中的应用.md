...
flowchart TD
  Start([目标])
  Start --> Goal{目标?}
  Goal -->|投资组合/风险/定价| Algo[QUBO 或混合算法 §各节]
  Algo --> App[§金融应用案例]
```
...
graph LR
  Ax[量子金融计算公设 §基本概念]
  QUBO[QUBO 正确性 §核心量子金融算法]
  Hybrid[混合算法正确性 §量子-经典混合算法]
  Ax --> QUBO
  QUBO --> Hybrid
```
...
flowchart TD
  Need([应用需求])
  Need --> App{需求类型?}
  App -->|投资组合优化/风险度量/期权定价| Meth[量子投资组合优化或量子-经典混合 §金融应用案例]
  Meth --> Impl[§金融应用案例]
```
